...
- `packages/sidecar-proto`: Design Jury sidecar business protocol. Owns app/mode/source constants, namespace validation, stamp descriptor/fields/flags, IPC message schema, status shapes, error semantics, and default product path constants.
- `packages/sidecar`: generic sidecar runtime primitives. Includes bootstrap, IPC transport, path/runtime resolution, launch env, and JSON runtime file helpers; it must not hard-code Design Jury app keys or IPC business messages.
...
- Do not hard-code Design Jury app/source/mode constants in `sidecar` or `platform`.
...
pnpm --filter @design-jury/host typecheck
pnpm --filter @design-jury/host test
pnpm --filter @design-jury/sidecar-proto typecheck
pnpm --filter @design-jury/sidecar-proto test
pnpm --filter @design-jury/sidecar typecheck
pnpm --filter @design-jury/sidecar test
pnpm --filter @design-jury/platform typecheck
pnpm --filter @design-jury/platform test
```
