...
- Multiple Claude config directories cause Design Jury's spawned Claude process to use different state than the user's terminal session.
...
- `401`, `apiKeySource: "none"`, missing/invalid token, or authentication failure: tell the user to run `claude`, use `/login`, then retry the same Design Jury run.
...
- Windows credential or WSL/native mismatch indicators: suggest re-authenticating in the same shell/environment used by Design Jury and checking Windows Credential Manager where applicable.
