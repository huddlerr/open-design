...
- `pnpm --filter @design-jury/web exec vitest run -c vitest.config.ts tests/components/ProjectView.run-isolation.test.tsx tests/components/ProjectView.run-cleanup.test.tsx tests/components/ProjectView.pendingPrompt.test.tsx` - passed.
- `pnpm --filter @design-jury/daemon exec vitest run -c vitest.config.ts tests/runs.test.ts` - passed.
- `pnpm --filter @design-jury/web typecheck` - passed.
- `pnpm guard` - passed.
