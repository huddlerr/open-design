...
```bash
cd /c/Users/ekada/OneDrive/Desktop/Githubcontributing/design-jury
git status
...
```bash
pnpm --filter @design-jury/web build 2>&1 | tail -20 > /tmp/web-baseline-build.txt
```
...
```bash
pnpm --filter @design-jury/contracts test critique.test.ts
```
...
```bash
pnpm --filter @design-jury/contracts test sse.test.ts
```
...
import type { PanelEvent } from '@design-jury/contracts/critique';
...
```bash
pnpm --filter @design-jury/daemon test parser.test.ts
```
...
import type { PanelEvent, PanelistRole } from '@design-jury/contracts/critique';
...
import { describe, expect, it } from 'vitest';
import { defaultCritiqueConfig } from '@design-jury/contracts/critique';
...
```bash
pnpm --filter @design-jury/daemon test scoreboard.test.ts
```
...
```ts
import type { CritiqueConfig, RoundDecision } from '@design-jury/contracts/critique';
...
```ts
import type { FallbackPolicy } from '@design-jury/contracts/critique';
...
```bash
pnpm --filter @design-jury/daemon test migrations.test.ts
```
...
```bash
pnpm --filter @design-jury/daemon test orchestrator.test.ts
```
...
} from '@design-jury/contracts/critique';
import { panelEventToSse } from '@design-jury/contracts/sse';
import { parseCritiqueStream } from './parser';
...
```ts
import { describe, expect, it } from 'vitest';
import { defaultCritiqueConfig, PROTOCOL_VERSION } from '@design-jury/contracts/critique';
...
import { type CritiqueConfig, PROTOCOL_VERSION } from '@design-jury/contracts/critique';
...
```bash
pnpm --filter @design-jury/web test discovery.test.ts
```
...
import type { CritiqueSseEvent } from '@design-jury/contracts/sse';
import type { PanelistRole } from '@design-jury/contracts/critique';
