...
Runtime Adapter is the daemon layer responsible for adapting local AI agent CLIs. It converts Design Jury's unified generation requests into the actual command-line invocations for each CLI, and converts CLI output into streaming events that the frontend can consume.
