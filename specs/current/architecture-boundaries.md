...
This document defines the architectural boundaries for the local Design Jury app. These boundaries are architectural constraints; some enforcement details can be implemented later through the relevant roadmap workstreams.

## Product Shape

Design Jury is a local-first application. The near-term Electron version is a shell around the same `apps/web` and `apps/daemon` architecture.
