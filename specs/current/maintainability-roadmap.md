...
| W3 | Partial | Migrate project-owned code to TypeScript | R1 | W2 for highest-value shared types | Daemon source and tests have most `@ts-nocheck` suppressions removed. High-conflict daemon core files are deferred for follow-up to avoid blocking parallel PRs: `apps/daemon/src/server.ts`, `apps/daemon/src/agents.ts`, `apps/daemon/src/projects.ts`, `apps/daemon/src/runs.ts`, and `apps/daemon/src/cli.ts`. Remaining work is to migrate those files, restore full daemon typecheck coverage, and rerun `pnpm --filter @design-jury/daemon typecheck`, `pnpm typecheck`, and `pnpm guard`. |
