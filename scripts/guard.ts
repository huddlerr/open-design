  ...
  if (isStylePolicySource(repositoryPath)) {
    for (const match of source.matchAll(defaultTailwindPaletteClassPattern)) {
      violations.push({
        ...
        match: match[0],
        reason: "default Tailwind palette classes must use Design Jury token utilities instead",
      });
    }
  }
    ...
    if (repositoryPath.endsWith(".css") && isHardcodedColorEnforcedPath(repositoryPath)) {
      for (const match of collectCssHardcodedColorMatches(source)) {
        ...
        addStylePolicyViolation(
          ...
          match.index,
          value,
          "unregistered hardcoded UI colors must use Design Jury tokens or an explicit allowlist entry",
        );
      }
    } else {
...
async function checkStylePolicy(): Promise<boolean> {
  ...
  if (violations.length > 0) {
    ...
    console.error("Use Design Jury token utilities/CSS variables or add a narrow allowlist entry with a reason.");
    return false;
  }
