...
function createPaths(root: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "Design Jury-second-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "Design Jury-second.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "Design Jury"),
    installedExePath: join(namespaceRoot, "runtime", "install", "Design Jury", "Design Jury.exe"),
    ...
    setupPath: join(namespaceRoot, "builder", "Design Jury-second-setup.exe"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "Design Jury", "Uninstall.exe"),
    ...
    unpackedExePath: join(namespaceRoot, "builder", "win-unpacked", "Design Jury.exe"),
...
describe("materializeCachedUnpackedForInstaller", () => {
  it("overwrites cached packaged config and app package version", async () => {
    const root = await mkdtemp(join(tmpdir(), "design-jury-win-builder-"));
    ...
    try {
      ...
      await writeFile(join(cachedUnpackedRoot, "Design Jury.exe"), "exe\n", "utf8");
      await writeFile(
        join(cachedUnpackedRoot, "resources", "design-jury-config.json"),
      ...
      await writeFile(
        ...
        `${JSON.stringify({ name: "design-jury-packaged-app", version: "0.5.0-beta.1" })}\n`,
        "utf8",
      );
      ...
      await expect(readFile(join(paths.unpackedRoot, "Design Jury.exe"), "utf8")).resolves.toBe("exe\n");
      await expect(readFile(join(paths.unpackedRoot, "resources", "design-jury-config.json"), "utf8")).resolves.toContain(
        '"namespace":"second"',
      );
