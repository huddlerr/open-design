...
describe("mac standalone prebundle policy", () => {
  ...
  it("keeps server-mode package topology unchanged", () => {
    expect(
      shouldInstallInternalPackageForMacPrebundle({
        packageName: "@design-jury/web",
        webOutputMode: "server",
      }),
    ).toBe(true);
    expect(
      shouldInstallInternalPackageForMacPrebundle({
        packageName: "@design-jury/packaged",
        webOutputMode: "server",
      }),
    ).toBe(true);
  });
  ...
  it("excludes internal packages replaced by mac standalone prebundles", () => {
    for (const packageName of [
      "@design-jury/daemon",
      "@design-jury/desktop",
      "@design-jury/packaged",
      "@design-jury/platform",
      "@design-jury/sidecar",
      "@design-jury/sidecar-proto",
      "@design-jury/web",
    ]) {
      expect(
        shouldInstallInternalPackageForMacPrebundle({
    ...
    expect(
      shouldInstallInternalPackageForMacPrebundle({
        packageName: "@design-jury/contracts",
        webOutputMode: "standalone",
      }),
    ).toBe(true);
  });
...
describe("assertMacPrebundleMetafile", () => {
  it("accepts a safe web sidecar metafile", async () => {
    const root = await mkdtemp(join(tmpdir(), "design-jury-mac-prebundle-"));
  ...
  it("rejects a packaged main metafile that pulled in web runtime closure", async () => {
    ...
    try {
      await writeFile(
        metafilePath,
        JSON.stringify({ inputs: { "/repo/node_modules/@design-jury/web/dist/sidecar/index.js": {} } }),
        "utf8",
      );
...
describe("renderMacPackagedMainEntry", () => {
  it("renders the prebundled runtime entry shim", () => {
    ...
    expect(renderMacPackagedMainEntry(true)).not.toContain("@design-jury/packaged");
  });

  it("renders the package entry shim for non-prebundled mode", () => {
    expect(renderMacPackagedMainEntry(false)).toContain("@design-jury/packaged");
