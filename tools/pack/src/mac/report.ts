...
export async function collectMacSizeReport(
  ...
  return {
    ...
    tracked: {
      ...
      webNextCacheBytes: await sizePathBytes(join(appNodeModulesRoot, "@design-jury", "web", ".next", "cache")),
      webPackageBytes: await sizePathBytes(join(appNodeModulesRoot, "@design-jury", "web")),
      webPackageStandaloneBytes: await sizePathBytes(join(appNodeModulesRoot, "@design-jury", "web", ".next", "standalone")),
    },
