...
async function buildApiAttachmentContext(
  ...
  for (const attachment of attachments) {
    if (remaining <= 0) {
      blocks.push(
        '[Design Jury omitted remaining attached files because the attachment context budget was exhausted.]',
      );
      break;
    }
...
function clipAttachmentText(text: string, maxChars: number): string {
  ...
  return `${text.slice(0, maxChars)}\n\n[Design Jury truncated ${omitted} chars from this attachment before sending it to the API provider.]`;
}
