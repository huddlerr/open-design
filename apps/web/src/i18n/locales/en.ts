...
export const en: Dict = {
  ...
  'settings.onboardingCreateBody':
    'Describe the site, app, deck, image, or video you want. Design Jury will create a project and keep the work editable.',
  ...
  'settings.agentInstall.pathHint':
    'If you installed a CLI with npm or Homebrew and it still shows as not installed, ensure the tool\'s bin directory is on the PATH the Design Jury daemon inherits (Terminal vs GUI apps can differ on macOS). See QUICKSTART.md (section "Local agent CLI and PATH").',
  'settings.agentInstall.stepOpenLinks': 'Open Install or Docs for your preferred agent.',
  'settings.agentInstall.stepAuth':
    'Authenticate with the vendor CLI (sign in or add API credentials) before returning to Design Jury.',
  ...
  'settings.mcpServerHint': 'Expose Design Jury as an MCP server for your coding agent.',
  ...
  'settings.privacyHint': 'What data is shared with the Design Jury team',
  'settings.privacyConsentKicker': 'Help us improve Design Jury',
  'settings.privacyConsentLead': 'Design Jury can share usage data with our team to help us improve. This includes:',
  ...
  'settings.mcpHint': 'Let coding agents (Cursor, Claude Code, VS Code…) read your Design Jury projects directly.',
  'settings.mcpDaemonError':
    "Couldn't reach the local daemon to resolve install paths ({error}). Make sure Design Jury is running, then reopen this panel.",
  'settings.mcpBuildDaemon': 'Build the daemon first.',
  'settings.mcpNodeMissing': 'Node binary is missing.',
  'settings.mcpBuildHint':
    'apps/daemon/dist/cli.js is missing. Run `pnpm --filter @design-jury/daemon build` and refresh.',
  ...
  'settings.mcpCapabilityDefault':
    'Default to the project and file you have open in Design Jury, so you can say "build this in my app" without re-stating which design.',
  'settings.mcpRunningNote':
    'Design Jury must be running for MCP tool calls to succeed. If you started your coding agent before opening Design Jury, restart the agent so it can reach the live daemon.',
  ...
  'homeHero.chip.createPluginHint': 'Author a reusable Design Jury plugin and add it to My plugins.',
  ...
  'designSystemPicker.noPreview': 'No preview page. Design Jury Systems to view the full preview.',
  ...
  'pluginsHome.subtitle': 'Ready-to-use Design Jury workflows bundled with this runtime. Pick one to load a starter prompt, or browse the registry for more.',
  'pluginsHome.browseRegistry': 'Browse registry',
  'pluginsHome.count': '{filtered} of {total}',
  'pluginsHome.loadingCatalog': 'Loading catalog…',
  'pluginsHome.emptyCatalog': 'Catalog is empty. Bundled plugins ship with Design Jury and should appear here automatically — try restarting the daemon if this persists.',
  ...
  'pluginCard.contributeAria': 'Contribute {title} to Design Jury',
  'pluginCard.contributeTitle': 'Contribute plugin to Design Jury with a pull request',
  'integrations.kicker': 'Integration',
  'integrations.lede': 'Connect external systems, bring MCP tools into your agent loop, and use Design Jury from other IDEs, scripts, and automations.',
  ...
  'mcpClient.daemonError': 'Could not reach the local daemon. Make sure Design Jury is running, then reopen this panel.',
  'mcpClient.saveFailed': 'Save failed. Check that the daemon is running and try again.',
  'useEverywhere.modalAria': 'Use Design Jury everywhere',
  'useEverywhere.modalTitle': 'Use Design Jury everywhere',
  'useEverywhere.modalSubtitle': 'Drop Design Jury into any IDE, agent, or script — CLI, HTTP, MCP, and Skills. Use “Copy guide for an agent” and paste into Claude Code, Codex, Cursor, openclaw, or hermes to set up everything.',
  ...
  'useEverywhere.section.overview.heading': 'Design Jury works wherever your agent works',
  'useEverywhere.section.overview.intro': 'Design Jury is more than a window — it is a local privileged daemon (`od`) plus a Skills + Design-Systems + Atoms registry. Once it is running on your machine, any code agent (Claude Code, Codex, Cursor, OpenCode/openclaw, Hermes, your own script) can drive generations, inspect projects, and produce design artifacts through four interchangeable surfaces.',
  'useEverywhere.section.overview.bullet1': 'CLI — `od <command>` for headless scripts, CI, and shell automation.',
  'useEverywhere.section.overview.bullet2': 'MCP server — wires Design Jury as a Model Context Protocol server so any MCP-capable agent can list skills, run scenarios, and read artifacts.',
  ...
  'useEverywhere.section.overview.bullet4': 'Skills — drop-in `SKILL.md` packs (Claude-compatible) that any agent already on your PATH can invoke without Design Jury at all.',
  ...
  'useEverywhere.section.overview.footer': 'The daemon writes to `./.od/` (project-local) by default. Set `OD_DATA_DIR=~/.design-jury` to share data across projects.',
  'useEverywhere.section.cli.tab': 'CLI · od',
  'useEverywhere.section.cli.heading': 'Drive Design Jury from any shell',
  ...
  'useEverywhere.section.mcp.heading': 'Expose Design Jury as an MCP server to any coding agent',
  'useEverywhere.section.mcp.intro': 'Design Jury ships with a Model Context Protocol server (`od mcp`) that lets any MCP-capable client — Cursor, Claude Code, Antigravity, VS Code Copilot Chat, openclaw, hermes — discover Design Jury tools (list skills, render previews, generate media, run plugins) without shelling out manually. The daemon publishes a ready-to-paste install snippet via `GET /api/mcp/install-info` for each major client.',
  ...
  'useEverywhere.section.mcp.footer': 'In the Design Jury app, open Settings → Integrations to copy a client-specific install command (Cursor, Claude Code, Antigravity, VS Code) instead of editing JSON by hand.',
  'useEverywhere.section.http.tab': 'HTTP API',
  'useEverywhere.section.http.heading': 'Same REST + SSE surface the web UI uses',
  'useEverywhere.section.http.intro': 'The local daemon serves an HTTP API at `http://127.0.0.1:7456` (port configurable). Every endpoint the web UI calls is also fair game for your scripts. Streaming endpoints (chat turns, project runs) emit Server-Sent Events with the contract types in `@design-jury/contracts`.',
  ...
  'useEverywhere.section.http.footer': 'Pure TypeScript types for every request/response live in `@design-jury/contracts` — import them in your script for full autocomplete without wiring a generator.',
  'useEverywhere.section.skills.tab': 'Skills & headless',
  'useEverywhere.section.skills.heading': 'Drop-in Skills for any agent — even without Design Jury running',
  'useEverywhere.section.skills.intro': 'A Skill is a directory with a Claude-compatible `SKILL.md` (YAML front-matter + body). Design Jury extends the format with the `od:` namespace (`mode`, `preview`, `design_system`, `inputs`, …) so the same artifact can be used both inside Design Jury and by a vanilla agent like Claude Code, Codex, openclaw, or hermes. Discovery follows a precedence chain so projects can override their own skills.',
  ...
  'useEverywhere.section.skills.snippet1': 'Minimal SKILL.md (Claude-compatible front matter + Design Jury extras)',
  ...
  'entry.githubStarAria': 'Star Design Jury on GitHub',
  ...
  'preview.errorBody': 'The example HTML failed to fetch. Make sure Design Jury is running and try again.',
  'preview.retry': 'Retry',
  ...
  'pet.tabBuiltInHint': 'Curated companions bundled with Design Jury — pick one and adopt.',
  ...
  'pet.codexBundledTitle': 'Ships with Design Jury — no download needed.',
  ...
  'settings.memoryStarterProjectDesc': 'Design Jury v0.5 — chat-driven design editor',
  ...
  'updater.availableBody': 'Design Jury {version} is available. It will download before the installer can open.',
  ...
  'updater.installerOpenBody': 'The installer is open. Design Jury is quitting so you can finish the update.',
  ...
  'updater.quitButton': 'Quit Design Jury',
  'updater.quitFailedBody': 'The installer is open, but Design Jury could not quit. Quit Design Jury before replacing the app.',
  'updater.quitFailedTitle': 'Could not quit',
  'updater.quitting': 'Quitting...',
  'updater.ready': 'Update ready',
  'updater.readyGeneric': 'A new version is ready. Design Jury will close and open the installer.',
  'updater.readyVersion': 'Design Jury {version} is ready. Design Jury will close and open the installer.',
