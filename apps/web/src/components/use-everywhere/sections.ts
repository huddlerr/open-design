// Content fixtures for the "Use Design Jury everywhere" guide modal.
//
...
export const GUIDE_SECTIONS: GuideSection[] = [
  {
    id: 'overview',
    tabLabel: 'Overview',
    heading: 'Design Jury works wherever your agent works',
    intro:
      'Design Jury is more than a window — it is a local privileged daemon ' +
    ...
    bullets: [
      ...
      'MCP server — wires Design Jury as a Model Context Protocol server so any MCP-capable agent can list skills, run scenarios, and read artifacts.',
      'HTTP API — `http://127.0.0.1:7456/api/*` REST + SSE endpoints; the same surface the web UI uses.',
      'Skills — drop-in `SKILL.md` packs (Claude-compatible) that any agent already on your PATH can invoke without Design Jury at all.',
    ...
    snippets: [
      ...
      },
      {
        ...
        body:
          ...
          '# Then start Design Jury in the shell you normally use for dev:\n' +
          'pnpm tools-dev',
      },
    ],
    footer:
      'The daemon writes to `./.od/` (project-local) by default. Set ' +
      '`OD_DATA_DIR=~/.design-jury` to share data across projects.',
  },
  {
    id: 'cli',
    tabLabel: 'CLI · od',
    heading: 'Drive Design Jury from any shell',
    intro:
  ...
  },
  {
    id: 'mcp',
    tabLabel: 'MCP server',
    heading: 'Expose Design Jury as an MCP server to any coding agent',
    intro:
      'Design Jury ships with a Model Context Protocol server (`od mcp`) ' +
      'that lets any MCP-capable client — Cursor, Claude Code, Antigravity, ' +
      'VS Code Copilot Chat, openclaw, hermes — discover Design Jury tools ' +
    ...
    footer:
      'In the Design Jury app, open Settings → Integrations to copy a ' +
  ...
  },
  {
    ...
    intro:
      ...
      'Server-Sent Events with the contract types in `@design-jury/contracts`.',
    bullets: [
    ...
    footer:
      ...
      '`@design-jury/contracts` — import them in your script for full ' +
      'autocomplete without wiring a generator.',
  },
  {
    id: 'skills',
    tabLabel: 'Skills & headless',
    heading: 'Drop-in Skills for any agent — even without Design Jury running',
    intro:
      'A Skill is a directory with a Claude-compatible `SKILL.md` ' +
      '(YAML front-matter + body). Design Jury extends the format with the ' +
      '`od:` namespace (`mode`, `preview`, `design_system`, `inputs`, …) so ' +
      'the same artifact can be used both inside Design Jury and by a vanilla ' +
    ...
    snippets: [
      {
        label: 'Minimal SKILL.md (Claude-compatible front matter + Design Jury extras)',
        language: 'yaml',
        body:
          '---\n' +
      ...
      },
      {
        ...
        body:
          ...
          '# Shell 1: start Design Jury after ingesting.\n' +
          'pnpm tools-dev\n' +
