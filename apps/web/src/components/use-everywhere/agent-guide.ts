...
// everything it needs to install Design Jury, expose it as MCP, and
...
export interface AgentGuideOptions {
   ...
   */
  cliHint?: string;
  /** Optional Design Jury version/channel; surfaced in the header for support tickets. */
  versionHint?: string;
}
...
export function buildAgentGuideMarkdown(options: AgentGuideOptions = {}): string {
  ...
  lines.push('# Design Jury — agent setup guide');
  lines.push('');
  lines.push(
    'You are a coding agent (Claude Code, Codex, Cursor, openclaw, hermes, or similar). ' +
      'The user wants Design Jury wired into your workflow. Design Jury is a local ' +
  ...
  if (options.versionHint) {
    lines.push('');
    lines.push(`> Reported Design Jury version: \`${options.versionHint}\``);
  }
  lines.push('');
  ...
  lines.push('   ```');
  lines.push('');
  lines.push('   If it 404s or times out, ask the user to run `pnpm tools-dev` (dev) or open the Design Jury app (packaged).');
  lines.push('');
