...
export const PLUGIN_AUTHORING_PROMPT_TEMPLATE = [
  `Create an Design Jury plugin for: {{${PLUGIN_AUTHORING_GOAL_INPUT}}}.`,
  '',
  ...
  '',
  '**Do NOT** suggest follow-up CLI commands such as `od plugin publish`, `od plugin publish --to design-jury`, `gh repo create`, `git init` / `git remote add` / `git push`, or any other publish / repo wiring. The plugin-folder card under Design Files already exposes three buttons whose prompts drive those flows end-to-end with the right auth gates, fallbacks, and retry rules baked in:',
  ...
  '- **Design Jury PR** — opens a draft PR against `nexu-io/design-jury` for the community catalog.',
  '',
