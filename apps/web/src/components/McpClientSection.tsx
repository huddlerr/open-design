...
//
// Design Jury connects to the configured servers as a CLIENT and surfaces
...
// so the most useful categories for Design Jury (visual generation, then
      ...
      {expanded ? (
        <>
          ...
          {isHttpLike && !row._isNew && row.id ? (
            usesManagedOAuth ? (
              ...
              <div className="mcp-oauth-hint hint">
                <strong>No managed OAuth.</strong> Design Jury will use this
          ...
          {isHttpLike && row._isNew && usesManagedOAuth ? (
            <div className="mcp-oauth-hint hint">
              Save first, then click <strong>Connect</strong> to grant Design Jury
              access via the provider's OAuth flow.
            </div>
          ) : null}
          {isHttpLike && row._isNew && !usesManagedOAuth ? (
            <div className="mcp-oauth-hint hint">
              <strong>No managed OAuth.</strong> Save this server and Design Jury
...
function McpOAuthControl({ serverId }: { serverId: string }) {
  ...
  useEffect(() => {
    ...
    if (typeof BroadcastChannel !== 'undefined') {
      bc = new BroadcastChannel('design-jury-mcp-oauth');
        ...
        {connected ? (
          ...
          <>
            ...
            <span>
              ...
              <span className="hint">
                Click Connect to grant Design Jury access via the provider's OAuth flow.
              </span>
            </span>
          </>
        )}
