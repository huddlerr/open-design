...
import type { OpenDesignHostUpdaterStatusListener, OpenDesignHostUpdaterStatusSnapshot } from '@design-jury/host';
...
function downloadedStatus(overrides: Partial<OpenDesignHostUpdaterStatusSnapshot> = {}): OpenDesignHostUpdaterStatusSnapshot {
  return {
    ...
    downloadPath: '/tmp/design-jury-updater/Design Jury Beta.dmg',
    state: 'downloaded',
    ...overrides,
  };
}
...
describe('UpdaterPopup', () => {
  ...
  it('shows only the ready indicator until the user opens the install prompt', async () => {
    ...
    expect(screen.getByText('Design Jury 1.2.3-beta.4 is ready. Design Jury will close and open the installer.')).toBeTruthy();
  ...
  it('uses localized ready prompt copy from the app i18n provider', async () => {
    ...
    expect(screen.getByText('Design Jury 1.2.3-beta.4 已就绪。Design Jury 会关闭并打开安装器。')).toBeTruthy();
  });
    ...
    await act(async () => {
      status = downloadedStatus({
        installResult: {
          ...
          path: '/tmp/design-jury-updater/Design Jury Beta.dmg',
        },
      });
