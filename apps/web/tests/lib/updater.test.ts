...
function downloadedStatus(overrides: Partial<OpenDesignHostUpdaterStatusSnapshot> = {}): OpenDesignHostUpdaterStatusSnapshot {
  return {
    arch: 'arm64',
    artifact: {
      name: 'Design Jury Beta.dmg',
      platformKey: 'macAppleSilicon',
      type: 'dmg',
      url: 'https://fixture.test/Design Jury Beta.dmg',
    },
    ...
    const model = deriveUpdaterModel(
      downloadedStatus({
        incoming: {
          arch: 'arm64',
          artifact: {
            name: 'Design Jury Beta 1.2.3-beta.5.dmg',
            platformKey: 'macAppleSilicon',
            type: 'dmg',
            url: 'https://fixture.test/Design Jury Beta 1.2.3-beta.5.dmg',
          },
          channel: 'beta',
  ...
  it('routes status, check, download, install, and quit requests through host helpers with flexible payloads', async () => {
    ...
    const install = vi.fn(async () => downloadedStatus({
      installResult: {
        dryRun: true,
        openedAt: '2026-05-19T00:00:00.000Z',
        path: status.downloadPath ?? '/tmp/design-jury-updater/Design Jury Beta.dmg',
      },
    }));
