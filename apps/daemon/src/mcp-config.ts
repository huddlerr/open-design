...
//
// Design Jury acts as an MCP CLIENT to one or more external MCP servers
...
// users can copy-paste between Design Jury and other tools without
// translation.
 ...
 * Design Jury daemon hand servers to a single `opencode run` invocation
...
export const MCP_TEMPLATES: McpTemplate[] = [
  // ── image-generation ────────────────────────────────────────────────
  {
    ...
    description:
      'Image and video generation MCP from higgsfield.ai. Exposes Soul, Nano Banana, Flux, Kling, Veo, Seedance, and 25+ other models. Endpoint is streamable HTTP at /mcp; click "Connect" after saving — Design Jury completes OAuth and stores the token server-side, so no terminal step is needed and the connection survives across chat turns and cloud deployments.',
    transport: 'http',
    authMode: 'oauth',
  ...
  },
  {
    ...
    example:
      'Generate a 1200×630 dark-theme OG image titled "Design Jury 1.0" with a subtitle "Design with agents", with a Lucide "sparkles" icon.',
    command: 'npx',
    args: ['-y', 'ogforge-api'],
  },
  {
  ...
  // ── utilities ───────────────────────────────────────────────────────
  {
    ...
    label: 'Filesystem',
    description:
      'Read, write and list files in a sandboxed directory. Useful for letting the agent operate on a folder outside your Design Jury project.',
    transport: 'stdio',
    category: 'utilities',
