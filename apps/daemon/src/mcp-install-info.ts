...
// and free of @design-jury/sidecar-proto so it can be unit-tested
...
export function buildMcpInstallPayload(
  ...
  if (!inputs.cliExists) {
    hints.push(
      `Design Jury CLI entry is missing at ${inputs.cliPath}. Rebuild the daemon or packaged app and refresh.`,
    );
  }
  if (!inputs.nodeExists) {
    hints.push(
      `Node-compatible runtime at ${inputs.execPath} no longer exists. Reinstall Design Jury or Node and restart the daemon.`,
    );
  }
