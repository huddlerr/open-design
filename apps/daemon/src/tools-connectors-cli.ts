...
Environment:
  ...
  OD_BIN          Design Jury CLI script for agent wrapper invocations
...
export async function auditDesignSystemPackage(
  ...
  if (options.referencePackage === true) {
    if (!fileSet.has('DESIGN.md')) {
      addIssue('warning', 'missing_open_design_rules', 'Reference packages may omit DESIGN.md, but generated Design Jury packages must include it as the canonical rules file.', 'DESIGN.md');
    }
  } else {
