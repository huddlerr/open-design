...
//   - design-jury            → nexu-io/design-jury (plugins/community/<plugin-name>/).
  ...
  switch (args.catalog) {
    ...
    case 'design-jury': {
      const bodyWithRegistry = [
        body,
        '',
        '## Design Jury registry entry',
        '',
        '- Target path: `plugins/community/<plugin-name>/design-jury.json`',
        '- Generated index: `plugins/registry/community/design-jury-marketplace.json`',
        '- Required checks: `od plugin validate`, `od plugin pack`, integrity digest, preview smoke.',
      ].join('\n');
      const url = newIssueUrl('nexu-io/design-jury', title, bodyWithRegistry);
      return {
        catalog: args.catalog,
        catalogLabel: 'nexu-io/design-jury',
        url,
...
export function upsertMarketplaceJsonEntry(args: {
  ...
  const manifest: MarketplaceJsonManifest = {
    ...
    name: typeof existing.name === 'string' ? existing.name : 'design-jury-marketplace',
...
function renderPrBody(m: PublishMetadata): string {
  ...
  lines.push('- Ships `design-jury.json` sidecar (additive Design Jury metadata).');
  lines.push('');
  lines.push('Generated by `od plugin publish` — see https://design-jury.ai/docs/plugins-spec.md.');
  return lines.join('\n');
}
