...
async function migrateLegacyDesignSystemPackage(
  ...
  const title = normalizeTitle(metadata.title ?? firstHeading(body) ?? id);
  const summary = summarize(body) || 'A reusable Design Jury design system.';
  ...
  await Promise.all([
    ...
    appKitExists
      ? writeIfMissing(
          ...
          `# ${title} UI Kit\n\nThis package was migrated from an earlier Design Jury design-system workspace. Use \`index.html\` as the applied interface example and replace it with source-backed modular components when new repository evidence is available.\n`,
        )
...
async function writeGeneratedDesignSystemFiles(
  ...
  const summary = input.summary || 'A user-created Design Jury design system.';
...
function buildDraftDesignSystemBody(input: UserDesignSystemInput & { title: string }): string {
  ...
  const summary = cleanText(input.summary) || 'A user-authored design system for future Design Jury projects.';
...
A reusable Design Jury package for ${input.title}.
...
function renderSkill(input: {
  ...
  return `---
name: ${skillName}
description: Use this skill when generating Design Jury artifacts that should follow ${input.title}.
user-invocable: true
---
...
function renderOverviewHtml(
  ...
  return renderHtmlDocument(
    title,
    `<main class="overview">
      <p class="eyebrow">Design Jury system</p>
