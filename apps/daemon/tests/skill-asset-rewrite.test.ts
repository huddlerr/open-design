...
describe('rewriteSkillAssetUrls', () => {
  it('rewrites ./assets/<file> img sources to the daemon route', () => {
    ...
    expect(rewriteSkillAssetUrls(html, 'design-jury-landing')).toBe(
      `<img src='/api/skills/design-jury-landing/assets/hero.png' alt='' />`,
    );
  });
  ...
  it('rewrites sibling skill asset references', () => {
    const html = `<img src='../design-jury-landing/assets/hero.png' /><a href="../skill-two/assets/guide.pdf"></a>`;
    expect(rewriteSkillAssetUrls(html, 'foo')).toBe(
      `<img src='/api/skills/design-jury-landing/assets/hero.png' /><a href="/api/skills/skill-two/assets/guide.pdf"></a>`,
    );
  });
