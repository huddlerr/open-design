...
import { createJsonIpcServer, type JsonIpcServerHandle } from "@design-jury/sidecar";
import { SIDECAR_ENV, SIDECAR_MESSAGES } from "@design-jury/sidecar-proto";
...
describe("resolveDaemonUrl", () => {
  ...
  it("discovers the live daemon URL via the concrete sidecar IPC status endpoint", async () => {
    const socketPath = process.platform === "win32"
      ? `\\\\.\\pipe\\design-jury-daemon-url-${process.pid}-${Date.now()}`
