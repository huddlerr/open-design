...
import { bootstrapSidecarRuntime, createJsonIpcServer, resolveAppIpcPath } from "@design-jury/sidecar";
...
function resolveHeadlessNamespaceBaseRoot(): string {
  ...
  return join(dataBase, "design-jury", "namespaces");
}
...
function resolveHeadlessConfig(): PackagedConfig {
  ...
  // sibling design-jury/ directory relative to the node_modules that contain
  // this file — the layout written by tools-pack linux headless-install.
  const resourceRoot =
    process.env.OD_RESOURCE_ROOT ??
    join(__dirname, "..", "..", "..", "design-jury");
...
void main().catch((error: unknown) => {
  process.stderr.write(
    `design-jury headless failed: ${error instanceof Error ? error.message : String(error)}\n`,
  );
  process.exit(1);
});
