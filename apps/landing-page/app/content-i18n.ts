...
const CONTENT_COPY: Record<Exclude<LandingLocaleCode, 'en'>, ContentCopy> = {
  zh: {
    ...
    blogNoun: '文章',
    unknownTag: '分类',
    skillDescription: (name, labels) => `${name} 是一个可组合的 Design Jury Skill，用于${labels.join('、') || '设计产出'}工作流；可由本地代理调用，并和仓库中的设计系统一起复用。`,
    ...
    craftSummary: (name) => `这条 Design Jury 工艺规则定义 ${name} 的执行标准，帮助代理在生成 artifact 时保持一致、可读和可交付。`,
    templateName: (name) => `${name}模板`,
    templateSummary: (name) => `${name} 是可复用的 Design Jury Live Artifact 模板，包含渲染入口、示例数据和可 fork 的文件结构。`,
    pluginTitle: (kind, id) => `${kind}插件 · ${id}`,
    pluginDescription: (kind, labels) => `用于${kind}工作流的 Design Jury 插件。安装后可在本地 daemon 和 od CLI 中复用${labels.length ? `，覆盖${labels.join('、')}` : ''}。`,
    pluginExample: (kind) => `使用该插件创建一个${kind}任务，并在本地 Design Jury 工作区中查看生成结果。`,
    blogTitle: (topic) => `Design Jury 指南：${topic}`,
    blogSummary: (topic) => `这篇本地化摘要说明 ${topic} 与 Design Jury 的本地优先、BYOK 和可组合 Skill 工作流之间的关系。`,
    blogBody: (topic, summary) => `<p>${summary}</p><h2>本地化摘要</h2><p>这篇文章围绕 ${topic} 展开，说明 Design Jury 如何把设计 artifact、Skill、设计系统和本地代理工作流连接起来。</p><p>当前页面使用站内 i18n fallback 渲染本地化正文；完整人工翻译可继续通过 frontmatter 的 <code>i18n.bodyHtml</code> 覆盖。</p>`,
  },
  'zh-tw': {
    skillNoun: 'Skill',
    systemNoun: '設計系統',
    ...
    blogNoun: '文章',
    unknownTag: '分類',
    skillDescription: (name, labels) => `${name} 是一個可組合的 Design Jury Skill，用於${labels.join('、') || '設計產出'}工作流；可由本地代理呼叫，並和 repo 中的設計系統一起複用。`,
    ...
    craftSummary: (name) => `這條 Design Jury 工藝規則定義 ${name} 的執行標準，幫助代理在生成 artifact 時保持一致、可讀和可交付。`,
    templateName: (name) => `${name}模板`,
    templateSummary: (name) => `${name} 是可複用的 Design Jury Live Artifact 模板，包含渲染入口、示例資料和可 fork 的檔案結構。`,
    pluginTitle: (kind, id) => `${kind}外掛 · ${id}`,
    pluginDescription: (kind, labels) => `用於${kind}工作流的 Design Jury 外掛。安裝後可在本地 daemon 和 od CLI 中複用${labels.length ? `，覆蓋${labels.join('、')}` : ''}。`,
    pluginExample: (kind) => `使用該外掛建立一個${kind}任務，並在本地 Design Jury 工作區中查看生成結果。`,
    blogTitle: (topic) => `Design Jury 指南：${topic}`,
    blogSummary: (topic) => `這篇本地化摘要說明 ${topic} 與 Design Jury 的本地優先、BYOK 和可組合 Skill 工作流之間的關係。`,
    blogBody: (topic, summary) => `<p>${summary}</p><h2>本地化摘要</h2><p>這篇文章圍繞 ${topic} 展開，說明 Design Jury 如何把設計 artifact、Skill、設計系統和本地代理工作流連接起來。</p><p>目前頁面使用站內 i18n fallback 渲染本地化正文；完整人工翻譯可繼續透過 frontmatter 的 <code>i18n.bodyHtml</code> 覆蓋。</p>`,
  },
  ja: {
    skillNoun: 'スキル',
    systemNoun: 'デザインシステム',
    ...
    blogNoun: '記事',
    unknownTag: '分類',
    skillDescription: (name, labels) => `${name} は、${labels.join('、') || 'デザイン制作'}のための Design Jury スキルです。ローカルエージェントから呼び出せ、リポジトリ内のデザインシステムと一緒に再利用できます。`,
    ...
    templateSummary: (name) => `${name} は再利用可能な Design Jury Live Artifact テンプレートで、レンダー入口、サンプルデータ、fork 可能な構成を含みます。`,
    pluginTitle: (kind, id) => `${kind} プラグイン · ${id}`,
    pluginDescription: (kind, labels) => `${kind} ワークフロー向けの Design Jury プラグインです。インストール後はローカル daemon と od CLI から再利用できます${labels.length ? `。対象: ${labels.join('、')}` : ''}。`,
    pluginExample: (kind) => `このプラグインで ${kind} タスクを作成し、ローカルの Design Jury ワークスペースで結果を確認します。`,
    blogTitle: (topic) => `Design Jury ガイド: ${topic}`,
    blogSummary: (topic) => `${topic} と、Design Jury のローカルファースト、BYOK、構成可能なスキルワークフローの関係をまとめます。`,
    blogBody: (topic, summary) => `<p>${summary}</p><h2>ローカライズ概要</h2><p>この記事は ${topic} を起点に、Design Jury が artifact、スキル、デザインシステム、ローカルエージェントをどう接続するかを説明します。</p><p>このページは i18n fallback で本文を表示しています。完全な人手翻訳は frontmatter の <code>i18n.bodyHtml</code> で上書きできます。</p>`,
  },
  ko: {
    skillNoun: '스킬',
    systemNoun: '디자인 시스템',
    ...
    blogNoun: '글',
    unknownTag: '분류',
    skillDescription: (name, labels) => `${name}은 ${labels.join(', ') || '디자인 산출물'} 워크플로를 위한 조합 가능한 Design Jury 스킬입니다. 로컬 에이전트가 호출하고 저장소의 디자인 시스템과 함께 재사용할 수 있습니다.`,
    ...
    templateSummary: (name) => `${name}은 재사용 가능한 Design Jury Live Artifact 템플릿이며 렌더링入口, 샘플 데이터, fork 가능한 구조를 포함합니다.`,
    pluginTitle: (kind, id) => `${kind} 플러그인 · ${id}`,
    pluginDescription: (kind, labels) => `${kind} 워크플로용 Design Jury 플러그인입니다. 설치 후 로컬 daemon과 od CLI에서 재사용할 수 있습니다${labels.length ? `: ${labels.join(', ')}` : ''}.`,
    pluginExample: (kind) => `이 플러그인으로 ${kind} 작업을 만들고 로컬 Design Jury 워크스페이스에서 결과를 확인합니다.`,
    blogTitle: (topic) => `Design Jury 가이드: ${topic}`,
    blogSummary: (topic) => `${topic}이 Design Jury의 로컬 우선, BYOK, 조합 가능한 스킬 워크플로와 어떻게 연결되는지 요약합니다.`,
    blogBody: (topic, summary) => `<p>${summary}</p><h2>현지화 요약</h2><p>이 글은 ${topic}을 중심으로 Design Jury이 artifact, 스킬, 디자인 시스템, 로컬 에이전트를 어떻게 연결하는지 설명합니다.</p><p>현재 본문은 i18n fallback으로 렌더링됩니다. 완전한 번역은 frontmatter의 <code>i18n.bodyHtml</code>로 덮어쓸 수 있습니다.</p>`,
  },
  de: {
    skillNoun: 'Skill',
    ...
    blogSummary: (topic) => `Lokalisierte Zusammenfassung zu ${topic} und dem lokalen BYOK-Skill-Workflow von Design Jury.`,
    blogBody: (topic, summary) => `<p>${summary}</p><h2>Lokalisierte Zusammenfassung</h2><p>Dieser Beitrag erklaert, wie Design Jury ${topic} mit Artefakten, Skills, Designsystemen und lokalen Agenten verbindet.</p><p>Der Text nutzt aktuell einen i18n-Fallback. Eine vollstaendige Uebersetzung kann ueber <code>i18n.bodyHtml</code> im Frontmatter hinterlegt werden.</p>`,
  },
  fr: {
    skillNoun: 'skill',
    ...
    skillDescription: (name, labels) => `${name} est un skill Design Jury composable pour les flux ${labels.join(', ') || 'de production design'}. Il s'execute avec l'agent local et se reutilise avec les systemes DESIGN.md.`,
    ...
    craftSummary: (name) => `Cette regle Design Jury definit les standards ${name} pour produire des artefacts coherents, lisibles et livrables.`,
    ...
    pluginDescription: (kind, labels) => `Plugin Design Jury pour les flux ${kind}. Une fois installe, il fonctionne avec le daemon local et la CLI od${labels.length ? `; portee: ${labels.join(', ')}` : ''}.`,
    pluginExample: (kind) => `Utilisez ce plugin pour lancer une tache ${kind} et verifier le resultat dans l'espace de travail Design Jury local.`,
    blogTitle: (topic) => `Guide Design Jury : ${topic}`,
    blogSummary: (topic) => `Resume localise de ${topic} dans le contexte local-first, BYOK et skills composables d'Design Jury.`,
    blogBody: (topic, summary) => `<p>${summary}</p><h2>Resume localise</h2><p>Cet article explique comment Design Jury relie ${topic}, les artefacts, les skills, les systemes de design et les agents locaux.</p><p>Cette page utilise un fallback i18n. Une traduction complete peut etre fournie via <code>i18n.bodyHtml</code> dans le frontmatter.</p>`,
  },
  ru: {
    skillNoun: 'навык',
    ...
    skillDescription: (name, labels) => `${name} — составной навык Design Jury для сценариев ${labels.join(', ') || 'дизайн-артефактов'}. Его запускает локальный агент, а правила DESIGN.md переиспользуются между задачами.`,
    ...
    craftSummary: (name) => `Это правило Design Jury задает стандарт ${name}, чтобы агент создавал согласованные и пригодные к передаче артефакты.`,
    ...
    pluginDescription: (kind, labels) => `Плагин Design Jury для сценариев ${kind}. После установки доступен локально через daemon и CLI od${labels.length ? `; охват: ${labels.join(', ')}` : ''}.`,
    pluginExample: (kind) => `Создайте задачу ${kind} с этим плагином и проверьте результат в локальном рабочем пространстве Design Jury.`,
    blogTitle: (topic) => `Гид Design Jury: ${topic}`,
    blogSummary: (topic) => `Локализованное резюме о ${topic} и о том, как это связано с local-first, BYOK и составными навыками Design Jury.`,
    blogBody: (topic, summary) => `<p>${summary}</p><h2>Локализованное резюме</h2><p>Статья объясняет, как Design Jury связывает ${topic}, артефакты, навыки, дизайн-системы и локальных агентов.</p><p>Сейчас страница использует i18n fallback. Полный перевод можно задать через <code>i18n.bodyHtml</code> во frontmatter.</p>`,
  },
  es: {
    skillNoun: 'skill',
    ...
    skillDescription: (name, labels) => `${name} es un skill componible de Design Jury para flujos de ${labels.join(', ') || 'artefactos de diseño'}. Lo ejecuta el agente local y reutiliza sistemas DESIGN.md.`,
    ...
    craftSummary: (name) => `Esta regla de Design Jury define el estandar ${name} para producir artefactos coherentes, legibles y entregables.`,
    ...
    pluginDescription: (kind, labels) => `Plugin de Design Jury para flujos de ${kind}. Tras instalarlo, funciona con el daemon local y la CLI od${labels.length ? `; cubre: ${labels.join(', ')}` : ''}.`,
    pluginExample: (kind) => `Usa este plugin para crear una tarea de ${kind} y revisar el resultado en el workspace local de Design Jury.`,
    blogTitle: (topic) => `Guia Design Jury: ${topic}`,
    blogSummary: (topic) => `Resumen localizado sobre ${topic} dentro del flujo local-first, BYOK y de skills componibles de Design Jury.`,
    blogBody: (topic, summary) => `<p>${summary}</p><h2>Resumen localizado</h2><p>Este articulo explica como Design Jury conecta ${topic}, artefactos, skills, sistemas de diseño y agentes locales.</p><p>La pagina usa un fallback i18n; una traduccion completa puede sobrescribirse con <code>i18n.bodyHtml</code> en el frontmatter.</p>`,
  },
  'pt-br': {
    skillNoun: 'skill',
    ...
    blogNoun: 'artigo',
    unknownTag: 'categoria',
    skillDescription: (name, labels) => `${name} e um skill componivel do Design Jury para fluxos de ${labels.join(', ') || 'artefatos de design'}. Ele roda com o agente local e reutiliza sistemas DESIGN.md.`,
    ...
    craftSummary: (name) => `Esta regra do Design Jury define o padrao ${name} para gerar artefatos consistentes, legiveis e entregaveis.`,
    ...
    pluginTitle: (kind, id) => `Plugin de ${kind} · ${id}`,
    pluginDescription: (kind, labels) => `Plugin do Design Jury para fluxos de ${kind}. Depois de instalado, funciona no daemon local e na CLI od${labels.length ? `; cobre: ${labels.join(', ')}` : ''}.`,
    pluginExample: (kind) => `Use este plugin para criar uma tarefa de ${kind} e revisar o resultado no workspace local do Design Jury.`,
    blogTitle: (topic) => `Guia Design Jury: ${topic}`,
    blogSummary: (topic) => `Resumo localizado sobre ${topic} no fluxo local-first, BYOK e de skills componiveis do Design Jury.`,
    blogBody: (topic, summary) => `<p>${summary}</p><h2>Resumo localizado</h2><p>Este artigo explica como o Design Jury conecta ${topic}, artefatos, skills, sistemas de design e agentes locais.</p><p>A pagina usa um fallback i18n; uma traducao completa pode ser sobrescrita por <code>i18n.bodyHtml</code> no frontmatter.</p>`,
  },
  it: {
    skillNoun: 'skill',
    ...
    blogNoun: 'articolo',
    unknownTag: 'categoria',
    skillDescription: (name, labels) => `${name} e uno skill componibile di Design Jury per flussi ${labels.join(', ') || 'di artefatti design'}. Viene eseguito dall'agente locale e riusa sistemi DESIGN.md.`,
    ...
    craftSummary: (name) => `Questa regola Design Jury definisce lo standard ${name} per produrre artefatti coerenti, leggibili e consegnabili.`,
    ...
    pluginTitle: (kind, id) => `Plugin ${kind} · ${id}`,
    pluginDescription: (kind, labels) => `Plugin Design Jury per flussi ${kind}. Dopo l'installazione funziona con il daemon locale e la CLI od${labels.length ? `; copre: ${labels.join(', ')}` : ''}.`,
    pluginExample: (kind) => `Usa questo plugin per creare un task ${kind} e controllare il risultato nel workspace locale Design Jury.`,
    blogTitle: (topic) => `Guida Design Jury: ${topic}`,
    blogSummary: (topic) => `Sintesi localizzata di ${topic} nel flusso local-first, BYOK e skill componibili di Design Jury.`,
    blogBody: (topic, summary) => `<p>${summary}</p><h2>Sintesi localizzata</h2><p>Questo articolo spiega come Design Jury collega ${topic}, artefatti, skill, sistemi di design e agenti locali.</p><p>La pagina usa un fallback i18n; una traduzione completa puo essere fornita con <code>i18n.bodyHtml</code> nel frontmatter.</p>`,
  },
  vi: {
    skillNoun: 'skill',
    ...
    skillDescription: (name, labels) => `${name} la skill Design Jury co the ghep noi cho luong ${labels.join(', ') || 'artifact thiet ke'}. Skill chay voi agent cuc bo va tai su dung cac he DESIGN.md.`,
    ...
    craftSummary: (name) => `Quy tac Design Jury nay dat chuan ${name} de agent tao artifact nhat quan, de doc va co the ban giao.`,
    ...
    pluginTitle: (kind, id) => `Plugin ${kind} · ${id}`,
    pluginDescription: (kind, labels) => `Plugin Design Jury cho luong ${kind}. Sau khi cai dat, plugin chay voi daemon cuc bo va CLI od${labels.length ? `; pham vi: ${labels.join(', ')}` : ''}.`,
    pluginExample: (kind) => `Dung plugin nay de tao tac vu ${kind} va xem ket qua trong workspace Design Jury cuc bo.`,
    blogTitle: (topic) => `Huong dan Design Jury: ${topic}`,
    blogSummary: (topic) => `Tom tat ban dia hoa ve ${topic} trong luong local-first, BYOK va skill co the ghep noi cua Design Jury.`,
    blogBody: (topic, summary) => `<p>${summary}</p><h2>Tom tat ban dia hoa</h2><p>Bai viet nay giai thich cach Design Jury ket noi ${topic}, artifact, skill, he thiet ke va agent cuc bo.</p><p>Trang dang dung fallback i18n; ban dich day du co the ghi de bang <code>i18n.bodyHtml</code> trong frontmatter.</p>`,
  },
  pl: {
    skillNoun: 'skill',
    ...
    skillDescription: (name, labels) => `${name} to komponowalny skill Design Jury dla przeplywow ${labels.join(', ') || 'artefaktow designu'}. Dziala z lokalnym agentem i wykorzystuje systemy DESIGN.md.`,
    ...
    craftSummary: (name) => `Ta regula Design Jury definiuje standard ${name}, aby agent tworzyl spojne i gotowe do przekazania artefakty.`,
    ...
    pluginTitle: (kind, id) => `Plugin ${kind} · ${id}`,
    pluginDescription: (kind, labels) => `Plugin Design Jury dla przeplywow ${kind}. Po instalacji dziala lokalnie przez daemon i CLI od${labels.length ? `; zakres: ${labels.join(', ')}` : ''}.`,
    pluginExample: (kind) => `Utworz zadanie ${kind} tym pluginem i sprawdz wynik w lokalnym workspace Design Jury.`,
    blogTitle: (topic) => `Przewodnik Design Jury: ${topic}`,
    blogSummary: (topic) => `Zlokalizowane podsumowanie ${topic} w przeplywie local-first, BYOK i komponowalnych skillow Design Jury.`,
    blogBody: (topic, summary) => `<p>${summary}</p><h2>Zlokalizowane podsumowanie</h2><p>Ten artykul pokazuje, jak Design Jury laczy ${topic}, artefakty, skille, systemy projektowe i lokalnych agentow.</p><p>Strona uzywa fallbacku i18n; pelne tlumaczenie mozna podac przez <code>i18n.bodyHtml</code> we frontmatter.</p>`,
  },
  id: {
    skillNoun: 'skill',
    ...
    blogNoun: 'artikel',
    unknownTag: 'kategori',
    skillDescription: (name, labels) => `${name} adalah skill Design Jury yang dapat dikomposisi untuk alur ${labels.join(', ') || 'artifact desain'}. Skill ini berjalan lewat agen lokal dan memakai ulang sistem DESIGN.md.`,
    ...
    craftSummary: (name) => `Aturan Design Jury ini menetapkan standar ${name} agar agen menghasilkan artifact yang konsisten, terbaca, dan siap diserahkan.`,
    ...
    pluginTitle: (kind, id) => `Plugin ${kind} · ${id}`,
    pluginDescription: (kind, labels) => `Plugin Design Jury untuk alur ${kind}. Setelah dipasang, plugin berjalan di daemon lokal dan CLI od${labels.length ? `; cakupan: ${labels.join(', ')}` : ''}.`,
    pluginExample: (kind) => `Gunakan plugin ini untuk membuat tugas ${kind} dan memeriksa hasilnya di workspace Design Jury lokal.`,
    blogTitle: (topic) => `Panduan Design Jury: ${topic}`,
    blogSummary: (topic) => `Ringkasan lokal tentang ${topic} dalam alur local-first, BYOK, dan skill yang dapat dikomposisi di Design Jury.`,
    blogBody: (topic, summary) => `<p>${summary}</p><h2>Ringkasan lokal</h2><p>Artikel ini menjelaskan cara Design Jury menghubungkan ${topic}, artifact, skill, sistem desain, dan agen lokal.</p><p>Halaman ini memakai fallback i18n; terjemahan lengkap dapat ditimpa lewat <code>i18n.bodyHtml</code> di frontmatter.</p>`,
  },
  nl: {
    skillNoun: 'skill',
    ...
    blogNoun: 'artikel',
    unknownTag: 'categorie',
    skillDescription: (name, labels) => `${name} is een composeerbare Design Jury-skill voor ${labels.join(', ') || 'designartefacten'}. De lokale agent voert hem uit en hergebruikt DESIGN.md-systemen.`,
    ...
    craftSummary: (name) => `Deze Design Jury-regel definieert ${name}, zodat agenten consistente, leesbare en overdraagbare artefacten maken.`,
    ...
    pluginTitle: (kind, id) => `${kind}-plugin · ${id}`,
    pluginDescription: (kind, labels) => `Design Jury-plugin voor ${kind}-workflows. Na installatie werkt hij lokaal via de daemon en od CLI${labels.length ? `; bereik: ${labels.join(', ')}` : ''}.`,
    pluginExample: (kind) => `Gebruik deze plugin om een ${kind}-taak te maken en het resultaat in de lokale Design Jury-workspace te bekijken.`,
    blogTitle: (topic) => `Design Jury-gids: ${topic}`,
    blogSummary: (topic) => `Gelokaliseerde samenvatting van ${topic} binnen de local-first, BYOK en composeerbare skill-workflow van Design Jury.`,
    blogBody: (topic, summary) => `<p>${summary}</p><h2>Gelokaliseerde samenvatting</h2><p>Dit artikel legt uit hoe Design Jury ${topic}, artefacten, skills, designsystemen en lokale agenten verbindt.</p><p>Deze pagina gebruikt een i18n-fallback. Een volledige vertaling kan via <code>i18n.bodyHtml</code> in de frontmatter worden geplaatst.</p>`,
  },
  ar: {
    skillNoun: 'مهارة',
    ...
    blogNoun: 'مقال',
    unknownTag: 'تصنيف',
    skillDescription: (name, labels) => `${name} مهارة قابلة للتركيب في Design Jury لسير عمل ${labels.join('، ') || 'إنتاج التصميم'}. تعمل مع الوكيل المحلي وتعيد استخدام أنظمة DESIGN.md.`,
    ...
    pluginDescription: (kind, labels) => `إضافة Design Jury لسير عمل ${kind}. بعد التثبيت تعمل محليا عبر daemon و od CLI${labels.length ? `؛ النطاق: ${labels.join('، ')}` : ''}.`,
    pluginExample: (kind) => `استخدم هذه الإضافة لإنشاء مهمة ${kind} ومراجعة النتيجة في مساحة عمل Design Jury المحلية.`,
    blogTitle: (topic) => `دليل Design Jury: ${topic}`,
    blogSummary: (topic) => `ملخص محلي حول ${topic} ضمن سير Design Jury المحلي و BYOK والمهارات القابلة للتركيب.`,
    blogBody: (topic, summary) => `<p>${summary}</p><h2>ملخص محلي</h2><p>تشرح هذه المقالة كيف يصل Design Jury بين ${topic} والملفات والمهارات وأنظمة التصميم والوكلاء المحليين.</p><p>تعرض الصفحة حاليا نصا عبر i18n fallback؛ يمكن توفير ترجمة كاملة عبر <code>i18n.bodyHtml</code> في frontmatter.</p>`,
  },
  tr: {
    skillNoun: 'skill',
    ...
    blogNoun: 'yazi',
    unknownTag: 'kategori',
    skillDescription: (name, labels) => `${name}, ${labels.join(', ') || 'tasarim artifact'} akislarinda kullanilan birlesebilir bir Design Jury skillidir. Yerel ajanla calisir ve DESIGN.md sistemlerini yeniden kullanir.`,
    ...
    craftSummary: (name) => `Bu Design Jury kurali ${name} standardini belirler; ajanlarin tutarli, okunabilir ve teslim edilebilir artifact uretmesine yardim eder.`,
    ...
    pluginDescription: (kind, labels) => `${kind} akislari icin Design Jury eklentisi. Kurulumdan sonra yerel daemon ve od CLI ile calisir${labels.length ? `; kapsam: ${labels.join(', ')}` : ''}.`,
    pluginExample: (kind) => `Bu eklentiyle bir ${kind} gorevi olusturun ve sonucu yerel Design Jury workspace'inde inceleyin.`,
    blogTitle: (topic) => `Design Jury rehberi: ${topic}`,
    blogSummary: (topic) => `${topic} konusunu Design Jury'in local-first, BYOK ve birlesebilir skill akisi baglaminda ozetler.`,
    blogBody: (topic, summary) => `<p>${summary}</p><h2>Yerellestirilmis ozet</h2><p>Bu yazi Design Jury'in ${topic}, artifact, skill, tasarim sistemi ve yerel ajanlari nasil bagladigini aciklar.</p><p>Sayfa su anda i18n fallback kullanir. Tam ceviri frontmatter icindeki <code>i18n.bodyHtml</code> ile verilebilir.</p>`,
  },
  uk: {
    skillNoun: 'навичка',
    systemNoun: 'дизайн-система',
    ...
    skillDescription: (name, labels) => `${name} — компонована навичка Design Jury для сценаріїв ${labels.join(', ') || 'дизайн-артефактів'}. Її запускає локальний агент, а системи DESIGN.md можна перевикористовувати.`,
    ...
    craftSummary: (name) => `Це правило Design Jury задає стандарт ${name}, щоб агент створював послідовні й готові до передачі артефакти.`,
    templateName: (name) => `Шаблон ${name}`,
    ...
    pluginDescription: (kind, labels) => `Плагін Design Jury для сценаріїв ${kind}. Після встановлення працює локально через daemon і CLI od${labels.length ? `; охоплення: ${labels.join(', ')}` : ''}.`,
    pluginExample: (kind) => `Створіть завдання ${kind} цим плагіном і перевірте результат у локальному workspace Design Jury.`,
    blogTitle: (topic) => `Гід Design Jury: ${topic}`,
    blogSummary: (topic) => `Локалізоване резюме про ${topic} у local-first, BYOK і компонованому skill-процесі Design Jury.`,
    blogBody: (topic, summary) => `<p>${summary}</p><h2>Локалізоване резюме</h2><p>Стаття пояснює, як Design Jury поєднує ${topic}, артефакти, навички, дизайн-системи й локальних агентів.</p><p>Зараз сторінка використовує i18n fallback. Повний переклад можна задати через <code>i18n.bodyHtml</code> у frontmatter.</p>`,
  },
};
...
const BLOG_TOPIC_TITLES: Record<string, Partial<Record<Exclude<LandingLocaleCode, 'en'>, string>>> = {
  ...
  'port-figma-workflow-design-jury-plugin': {
    zh: '把 Figma 工作流迁移成 Design Jury 插件',
    'zh-tw': '把 Figma 工作流遷移成 Design Jury 外掛',
    ja: 'FigmaワークフローをDesign Juryプラグインへ移植する',
    ko: 'Figma 워크플로를 Design Jury 플러그인으로 옮기기',
    de: 'Figma-Workflows als Open-Design-Plugin portieren',
    fr: 'porter un workflow Figma en plugin Design Jury',
    ru: 'перенос Figma-процесса в плагин Design Jury',
    es: 'llevar un flujo de Figma a un plugin de Design Jury',
    'pt-br': 'migrar um fluxo do Figma para um plugin Design Jury',
    it: 'portare un workflow Figma in un plugin Design Jury',
    vi: 'chuyển quy trình Figma thành plugin Design Jury',
    pl: 'przenoszenie workflow Figma do pluginu Design Jury',
    id: 'memindahkan alur Figma menjadi plugin Design Jury',
    nl: 'een Figma-workflow omzetten naar een Design Jury-plugin',
    ar: 'نقل سير عمل Figma إلى إضافة Design Jury',
    tr: 'Figma akışını Design Jury eklentisine taşıma',
    uk: 'перенесення Figma-процесу в плагін Design Jury',
  },
  'why-we-built-design-jury-as-a-skill-layer': {
    zh: '为什么把 Design Jury 做成 Skill 层',
    'zh-tw': '為什麼把 Design Jury 做成 Skill 層',
    ja: 'Design JuryをSkillレイヤーとして作った理由',
    ko: 'Design Jury을 Skill 레이어로 만든 이유',
    de: 'warum Design Jury als Skill-Schicht gebaut wurde',
    fr: 'pourquoi Design Jury est une couche de skills',
    ru: 'почему Design Jury построен как слой навыков',
    es: 'por qué Design Jury se construyó como capa de skills',
    'pt-br': 'por que o Design Jury foi criado como camada de skills',
    it: 'perché Design Jury è stato costruito come livello di skill',
    vi: 'vì sao Design Jury được xây như một lớp skill',
    pl: 'dlaczego Design Jury powstał jako warstwa skill',
    id: 'mengapa Design Jury dibangun sebagai lapisan skill',
    nl: 'waarom Design Jury als skill-laag is gebouwd',
    ar: 'لماذا بنينا Design Jury كطبقة مهارات',
    tr: 'Design Jury neden bir skill katmanı olarak kuruldu',
    uk: 'чому Design Jury створено як шар навичок',
  },
};
