...
function buildUiCopy(t: UiText): DeepPartial<LandingUiCopy> {
  return {
    blog: {
      title: t.blogTitle,
      seoTitle: `${t.blogTitle} — Design Jury`,
    ...
    tutorials: {
      title: t.guides,
      seoTitle: `${t.guides} — Design Jury`,
      description: `${t.guides}: Design Jury / YouTube / ${t.skills} / ${t.systems} / ${t.community}.`,
      ...
      detailTitle: (title) => `${title} — Design Jury ${t.guides}`,
      localizedTitle: (_title, author) => `Design Jury ${t.guides}: ${author}`,
      localizedSummary: (_title, author, category) =>
        `${author} / ${category}: Design Jury / ${t.skills} / ${t.systems} / ${t.templates}.`,
      localizedBodyHtml: (_title, author, summary) =>
        `<p>${summary}</p><h2>${t.blogTitle}</h2><p>${author}: Design Jury / ${t.skills} / ${t.systems} / ${t.templates}. ${t.blogDescription}</p>`,
    },
    catalog: {
      breadcrumbLabel: t.breadcrumb,
      skills: {
        title: (count) => `${t.skills} — ${count} ${t.capabilities} | Design Jury`,
        ...
        featured: t.featured,
        allAria: t.allSkills,
        detailTitle: (name) => `${name} — Design Jury ${t.skill}`,
        detailFallbackDescription: (name) => `Design Jury ${t.skill}: ${name}.`,
      ...
      systems: {
        title: (count) => `${t.systems} — ${count} ${t.visualSystems} | Design Jury`,
        ...
        detailTitle: (name) => `${name} — Design Jury ${t.system}`,
        detailFallbackDescription: (name, category) => `Design Jury ${t.system}: ${name}, ${category}.`,
      ...
      templates: {
        title: (count) => `${t.templates} — ${count} ${t.artifactTemplates} | Design Jury`,
        ...
        detailTitle: (name) => `${name} — Design Jury ${t.template}`,
      ...
      craft: {
        title: (count) => `${t.craft} — ${count} ${t.principles} | Design Jury`,
        ...
        allAria: t.allCraft,
        detailTitle: (name) => `${name} — Design Jury ${t.craftRule}`,
        detailFallbackDescription: (name) => `Design Jury ${t.craftRule}: ${name}.`,
    ...
    plugins: {
      ...
      details: t.details,
      detailTitle: (title) => `${title} — Design Jury ${t.plugin}`,
...
const SHARED = {
  zh: {
    blogTitle: '博客', blogDescription: '理解、探索和构建 Design Jury 的本地化笔记。', blogCategories: '博客分类', all: '全部', product: '产品', guides: '指南', useCases: '使用场景', community: '社区', minRead: '分钟阅读', readMore: '继续阅读 →', read: '阅读 →', backToBlog: '← 返回博客', noEntries: '暂无文章。', noPosts: '这个分类还没有文章。', nextStep: '下一步', joinDiscord: '加入 Discord', viewSource: '在 GitHub 查看源码 ↗', downloadTitle: '下载桌面端', downloadBody: '试用开源设计工作台，查看 release notes，或加入社区。', downloadLabel: '下载桌面端 ↗', skillsTitle: '本地运行 Skill 工作流', skillsBody: '浏览工作流库，选择起点，并接入已有 agent。', skillsLabel: '浏览工作流 ↗', repoTitle: '查看 GitHub 实现', repoBody: '打开仓库查看源码、点 Star、fork 工作流或讨论下一步。', repoLabel: '打开仓库 ↗',
    ...
    pluginRegistryTitle: 'Design Jury 插件 — 官方与社区注册表', pluginRegistryDescription: (count: number) => `浏览 ${count} 个 Design Jury 插件。`, directoryRailRight: 'Design Jury 注册表 · 官方 · 社区', directoryRailLeft: 'vendor/plugin-name · marketplace.json', topbarTitle: 'OD / 注册表', topbarSubtitle: '公开索引', topbarMeta: '官方 · 社区 · 自托管', sourceJson: '源 JSON', heroLabel: '插件注册表 · 公共生态', heroTitle: '浏览带实时预览的 agent 原生设计插件。', heroBody: '发现可安装的工作流、deck、图像模板、设计系统和原子能力。', browseRegistry: '浏览注册表', communityMarketplace: '社区 marketplace.json', registryPreview: '注册表预览', installableEntries: '可安装条目', official: '官方', withPreview: '带预览', surfaces: '表面类型', availableSources: '可用来源', registryEntries: '注册表条目', searchPlugins: '搜索插件', searchPlaceholder: '搜索插件、工作流、vendor...', filtersLabel: '注册表筛选', visiblePlugins: '个可见插件', openDetails: (title: string) => `打开 ${title} 详情`, details: '详情', pluginDetailTitle: (title: string) => `${title} — Design Jury 插件`, pluginDetailDescription: (description: string, command: string) => `${description} 使用 ${command} 安装。`, pluginRail: (id: string) => `Design Jury 插件 · ${id}`, allPlugins: '全部插件', registry: '注册表', deprecated: '已弃用', yanked: '已下架', installFromRegistry: '从注册表安装', copy: '复制', copied: '已复制', select: '选择', previewAndFacts: '插件预览与信息', marketplaceJson: 'Marketplace JSON 文件', sourceRepository: '源仓库', homepage: '主页', interactivePreview: '交互预览', imagePreview: '图片预览', videoPoster: '视频封面', liveHtmlPreview: 'Live HTML 预览', trusted: '可信', restricted: '受限', pluginId: '插件 ID', version: '版本', license: '许可证', publisher: '发布方', notSpecified: '未指定', howItResolves: '解析方式', provenance: '注册表来源', provenanceBody: '该条目来自 marketplace catalog，并解析到下方传输来源。', workflowSurface: '工作流表面', directSourceFallback: '直接来源 fallback', howPeopleUseIt: '使用方式', examplePromptBody: '注册表条目包含可直接运行的提示词种子。', moreFrom: (registryName: string) => `更多来自 ${registryName}`, relatedPlugins: '相关插件',
  },
...
const zhTw: UiText = {
  ...SHARED.zh,
  blogTitle: '部落格',
  blogDescription: '理解、探索與建構 Design Jury 的本地化筆記。',
  ...
  plugin: '外掛',
  plugins: '外掛',
  pluginRegistryTitle: 'Design Jury 外掛 — 官方與社群註冊表',
  pluginRegistryDescription: (count: number) => `瀏覽 ${count} 個 Design Jury 外掛。`,
  directoryRailRight: 'Design Jury 註冊表 · 官方 · 社群',
  ...
  details: '詳情',
  pluginDetailTitle: (title: string) => `${title} — Design Jury 外掛`,
  pluginDetailDescription: (description: string, command: string) =>
    `${description} 使用 ${command} 安裝。`,
  pluginRail: (id: string) => `Design Jury 外掛 · ${id}`,
  allPlugins: '全部外掛',
  sourceJson: '來源 JSON',
  registry: '註冊表',
  deprecated: '已棄用',
...
const zhTutorialsCopy = {
  ...
  detailTitle: (title: string) => `${title} — Design Jury 教程`,
  localizedTitle: (_title: string, author: string) => `Design Jury 教程：${author}`,
  localizedSummary: (_title: string, author: string, category: string) =>
    `${author} 通过一支 ${category} 视频讲解 Design Jury 的本地优先设计工作流、Skill、设计系统与可复用模板。`,
  localizedBodyHtml: (_title: string, author: string, summary: string) =>
    `<p>${summary}</p><h2>本地化摘要</h2><p>这支视频来自 ${author}，围绕 Design Jury 的安装、能力演示、设计系统、Skill 与本地 Agent 工作流展开。页面保留原始视频来源，非英文页面使用站内 i18n 摘要，避免正文回退成英文。</p>`,
...
const zhTwTutorialsCopy = {
  ...
  detailTitle: (title: string) => `${title} — Design Jury 教學`,
  localizedTitle: (_title: string, author: string) => `Design Jury 教學：${author}`,
  localizedSummary: (_title: string, author: string, category: string) =>
...
function localizeOwnedUiText(locale: Exclude<LandingLocaleCode, 'en'>, text: UiText): UiText {
  ...
  return {
    ...text,
    blogDescription: `${text.blogTitle}: Design Jury / ${text.product} / ${text.guides} / ${text.useCases} / ${text.community}.`,
    viewSource: `GitHub · ${labels.source} ↗`,
    downloadTitle: `${labels.download} Design Jury Desktop`,
    downloadBody: `Design Jury Desktop / GitHub / Discord.`,
    downloadLabel: `${labels.download} ↗`,
    skillsTitle: `${text.skills} · Design Jury`,
    skillsBody: `${text.skills}: SKILL.md / daemon / DESIGN.md / ${text.systems}.`,
    skillsLabel: `${text.skills} ↗`,
    repoTitle: `GitHub · ${labels.repository}`,
    repoBody: `GitHub: Design Jury / ${text.skills} / ${text.templates} / ${text.systems}.`,
    repoLabel: `GitHub ↗`,
    ...
    pluginRegistryTitle: `${text.plugins} — Design Jury`,
    pluginRegistryDescription: (count: number) => `${count} ${text.plugins} · Design Jury.`,
    directoryRailRight: `Design Jury · ${text.official} · ${text.community}`,
    directoryRailLeft: `vendor/plugin-name · marketplace.json`,
    ...
    heroTitle: `${text.plugins} · Design Jury`,
    ...
    pluginDetailTitle: (title: string) => `${title} — Design Jury ${text.plugin}`,
    pluginDetailDescription: (description: string, command: string) => `${description} ${command}.`,
    pluginRail: (id: string) => `Design Jury ${text.plugin} · ${id}`,
...
const ko = cloneWith(jp, {
  ...
  pluginRegistryTitle: 'Design Jury 플러그인 — 공식 및 커뮤니티 레지스트리', pluginRegistryDescription: (count) => `${count}개의 Design Jury 플러그인을 탐색합니다.`, topbarTitle: 'OD / 레지스트리', topbarSubtitle: '공개 인덱스', topbarMeta: '공식 · 커뮤니티 · 자체 호스팅', sourceJson: '소스 JSON', heroLabel: '플러그인 레지스트리 · 공개 생태계', heroTitle: '실시간 preview가 있는 agent-native 디자인 플러그인을 탐색하세요.', browseRegistry: '레지스트리 보기', registryPreview: '레지스트리 preview', installableEntries: '설치 가능 항목', official: '공식', withPreview: 'preview 있음', availableSources: '사용 가능한 소스', registryEntries: '레지스트리 항목', searchPlugins: '플러그인 검색', searchPlaceholder: '플러그인, workflow, vendor 검색...', visiblePlugins: '표시 중인 플러그인', openDetails: (title) => `${title} 상세 열기`, details: '상세', pluginDetailTitle: (title) => `${title} — Design Jury 플러그인`, installFromRegistry: '레지스트리에서 설치', copy: '복사', copied: '복사됨', select: '선택', previewAndFacts: '플러그인 preview 및 정보', sourceRepository: '소스 저장소', homepage: '홈페이지', trusted: '신뢰됨', restricted: '제한됨', pluginId: '플러그인 ID', version: '버전', license: '라이선스', publisher: '게시자', notSpecified: '미지정', howItResolves: '해결 방식', provenance: '레지스트리 출처', workflowSurface: '워크플로 surface', howPeopleUseIt: '사용 방식', relatedPlugins: '관련 플러그인',
});

const de = cloneWith(jp, {
  blogTitle: 'Journal', blogDescription: 'Lokalisierte Notizen zum Verstehen, Erkunden und Bauen mit Design Jury.', blogCategories: 'Journal-Kategorien', all: 'Alle', product: 'Produkt', guides: 'Leitfäden', useCases: 'Anwendungsfälle', community: 'Gemeinschaft', minRead: 'Min. Lesezeit', readMore: 'Weiterlesen →', read: 'Lesen →', backToBlog: '← Zurück zum Journal', noEntries: 'Noch keine Einträge.', noPosts: 'In dieser Kategorie gibt es noch keine Beiträge.', nextStep: 'Nächster Schritt', joinDiscord: 'Discord beitreten', viewSource: 'Quelle auf GitHub ansehen ↗', downloadTitle: 'Desktop-Build herunterladen', downloadBody: 'Teste den offenen Design-Workspace, lies die Versionsnotizen oder komm in die Gemeinschaft.', downloadLabel: 'Desktop herunterladen ↗', skillsTitle: 'Skill-Workflow lokal ausführen', skillsBody: 'Wähle einen Workflow und verbinde ihn mit deinem Agenten.', skillsLabel: 'Workflows ansehen ↗', repoTitle: 'Implementierung auf GitHub ansehen', repoBody: 'Prüfe den Quellcode, gib einen Stern, forke Workflows oder diskutiere nächste Schritte.', repoLabel: 'Repository öffnen ↗',
