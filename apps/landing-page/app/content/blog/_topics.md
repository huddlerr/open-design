# Design Jury — blog topic backlog

_Last reviewed: 2026-05-22 (0.8.0 release scoring pass)_

This file is the single source of truth for what we're considering, drafting, and have shipped on the Design Jury blog. The leading underscore keeps Astro's content collection from treating it as a post.

Maintained by the `design-jury-blog-factory` skill (`~/.codex/skills/design-jury-blog-factory/SKILL.md`). Update on every scoring pass, draft start, and ship.

Scoring rubric (4 dimensions × 5 = 20):

- **Fit** — how naturally Design Jury slots into the post
...
| 2 | **Claude Design pricing — and the BYOK math** (A2) | Guides (comparison) | 5 | 4 | 4 | 5 | **18** | Anthropic launch pricing public ($20/$100/$200/Enterprise) | Cost-comparison companion to seed BYOK post. Run the same brief on Claude Pro vs Design Jury + BYOK; publish the token bill. High commercial intent. |
| 3 | How to ship a launch deck with Design Jury in 30 minutes | Use cases | 5 | 4 | 3 | 5 | **17** | own workflow | Re-uses `guizang-ppt` skill + a real shipped deck. Hero image = the deck cover. |
| 4 | BYOK with DeepSeek: a $0.10 design pass on Design Jury | Guides (BYOK) | 5 | 4 | 3 | 5 | **17** | DeepSeek search volume + own usage | Concrete cost numbers from a real run. Single-provider deep-dive companion to the "BYOK reality check" overview just shipped. |
| 5 | Design Jury on Windows — a working setup guide (and what's still rough) | Guides (tutorial) | 5 | 5 | 4 | 3 | **17** | gh issues #1620 #1611 #1610 #1559 #1581 | Pure search intent — Windows users are hitting these. Lists each known issue + workaround with linked GitHub issue numbers so it stays auditable. Needs to be revised whenever a Windows fix lands. The "BYOK reality check" post (shipped 2026-05-14) already covers the BYOK subset; this one widens to non-BYOK Windows pain. |
| 6 | **Claude Design vs Figma Make vs Design Jury** (A3) | Guides (comparison) | 4 | 5 | 4 | 4 | **17** | strong existing SERP for "claude design vs figma make" (Magic Patterns / XDA / CreateWith / claude-codex.fr); no vs-Open-Design content yet | Three-way comparison piggybacking on existing search heat. Use the existing comparison tables as evidence. |
...
| 8 | Inside Atelier Zero — designing the Design Jury landing page with the agent that ships it | Use cases | 5 | 3 | 4 | 4 | **16** | own dogfooding | Meta in the right way: the page you're reading was built this way. Hero = a real screen recording still. |
...
| 14 | **Using Design Jury with Claude Code — the open handoff to engineering** (C1) | Use cases | 5 | 4 | 3 | 4 | **16** | Anthropic users in OD overlap; Claude Code = handoff target in Claude Design tutorial | Catches "claude code design" searchers; pairs naturally with A1. |
...
| 16 | Deploying a pnpm monorepo Astro app to Vercel — the output directory gotcha | Guides (tutorial) | 4 | 4 | 3 | 4 | **15** | gh issue #1628 (live) | Pure how-to. Cover root vs `apps/web` Vercel project root, `vercel.json#outputDirectory`, and the actual Design Jury `vercel.json`. Useful long-tail SEO. |
...
| 18 | Taste memory for design agents — a community proposal worth shipping | Community (proposal) | 4 | 2 | 4 | 4 | **14** | gh issue #1637 (itsmeved24) | Profile the proposal, cite the author, do not promise we'll build it. Get itsmeved24's consent. Frames Design Jury as a place where serious proposals get serious replies. |
| 19 | **Should designers use Claude Design? A field test** (B3) | Use cases | 4 | 4 | 3 | 3 | **14** | own dogfooding | Run the same brief on Claude Design + Design Jury, publish both artefacts. Requires actually buying a Claude Pro seat for the test. |
| 20 | **From Claude Design to Design Jury — moving your design system** (C2) | Guides (tutorial) | 5 | 3 | 3 | 3 | **14** | migration intent (long-tail evergreen) | Step-by-step: export from Claude Design, fold tokens into a `DESIGN.md`, re-run the brief. Wait until we have a real migrator. |
| 21 | huashu, guizang, open-codesign — the lineage behind Design Jury's skill protocol | Community (lineage) | 4 | 2 | 3 | 4 | **13** | own history | Lower intent but high voice value. Worth writing once we have 4–5 posts in flight to anchor the Community channel. |
...
Re-scored 2026-05-22 against the live tag `design-jury-v0.8.0` (305 PRs · 75 contributors · 7 days; `c20d156`, published 22 May 12:43 UTC). The release supersedes the previously-queued #9 "auto-update on Windows + Linux" row — that announcement is now folded into the broader 0.8.0 plate.

| Dim | Score | Why |
|---|---|---|
| Fit | 5 | Every claim in the post maps to a real PR or skill folder shipping in 0.8.0 |
| Intent | 5 | "Design Jury 0.8.0 release notes / what's new" is a direct search intent |
...
Title chosen (Step 2.5 fast-track): **"Design Jury 0.8.0: everything is a plugin"** — declarative, hits the search intent, ≤ 60 chars.

Alternates considered:
- "Design Jury 0.8.0 ships the plugin engine, headless CLI, and auto-update" (subhead-shaped, too long for a slug)
...
Alternates considered:
- "Design Jury vs Claude Design — and which one to pick this quarter" (comparative angle)
...
| Designing for agent-driven UI patterns (chat-as-canvas, IM-as-editor) | Use cases | 11 | An Design Jury skill ships that produces one of these patterns end-to-end |
...
|---|---|---|---|---|---|---|---|---|
| 2026-05-13 | why-we-built-design-jury-as-a-skill-layer | Product (essay / manifesto) | 18 | pending GSC propagation (3–14d) | pending | — | 2026-06-13 | Seed post. Anchors the Product channel and the "skill layer, not a product" framing. GSC indexing requested via Issue #4 playbook. |
...
| 2026-05-18 | port-figma-workflow-design-jury-plugin | Use cases | 17 | pending (just shipped) | pending | — | 2026-06-18 | Use-case post from the 0.8.0-preview plugin call. Turns "port a Figma workflow" into a concrete `SKILL.md` + `design-jury.json` + validate/pack/publish path. CTA = `Try this workflow`. |
| 2026-05-22 | design-jury-0-8-0-everything-is-a-plugin | Product (announcement) | 19 | pending (just shipped) | pending | — | 2026-06-22 | Release announcement for `design-jury-v0.8.0` (tag `c20d156`, 305 PRs · 75 contributors · 7 days). Covers the three architectural shifts (plugin engine, headless-by-default, plugins create plugins) and the packaged-auto-update + design-system + media-provider plate. CTA = `Download desktop`. Supersedes queued #9 "auto-update on Windows + Linux" — that row is dropped now that the broader plate is live. hero: plate-15-plugin-engine-modular.png · gpt-image-2 via cursor-builtin |

## Dropped (with reason)

Keep this list short — only entries useful as guard-rails so we don't re-litigate them.

- **"AI is replacing designers" hot take** — failed fit filter rule 1 (no Design Jury surface) and rule 4 (no real CTA). Design Jury's voice is editorial, not anxiety-driven.
...
- `gh issue list --repo nexu-io/design-jury --state open --limit 30` — current pain, BYOK, contributor work
- `gh issue list --repo nexu-io/design-jury --state closed --limit 30 --search "closed:>=$(date -v-7d +%Y-%m-%d)"` — recent shipped wins worth narrating
- `gh issue list --repo nexu-io/design-jury --label blog --state all` — direct content requests
- https://github.com/nexu-io/design-jury/issues — same data via web for skim/triage
