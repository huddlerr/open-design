---
title: "How to port a Figma workflow into an Design Jury plugin"
date: 2026-05-18
category: "Use cases"
readingTime: 6
...
The 0.8.0-preview thread makes a sharper ask: port that muscle memory into a plugin. Not a panel bolted onto a canvas. Not a private script only one team can run. A reusable Design Jury workflow that an agent can pick up, execute, review, and hand off through the same local-first loop as any other design task.

This is the practical version of the [0.8.0-preview call for plugins](https://github.com/nexu-io/design-jury/discussions/1727). If your team has one repeatable design workflow today, this post shows what it looks like to turn it into a plugin-shaped contribution.
...
That is why Design Jury's [skill layer](/blog/why-we-built-design-jury-as-a-skill-layer/) matters. A plugin is not an opaque runtime extension. It is a folder of files: a `SKILL.md` contract, optional design systems, optional examples, and an `design-jury.json` sidecar that tells Design Jury how to display and apply the workflow.

## The Design Jury angle is portability

The plugin spec states the contract plainly: `SKILL.md` stays the executable agent contract, while `design-jury.json` adds marketplace metadata, input fields, defaults, previews, and context wiring.

That gives one workflow two lives. In Design Jury, it appears as a plugin with a preview, inputs, provenance, and a one-click "use" path. In Claude Code, Cursor, Codex, Gemini CLI, OpenClaw, or another skill catalog, the same folder still works as a plain agent skill because the core behavior lives in Markdown.
...
| Design Jury metadata | `design-jury.json` |
...
3. **Add the Design Jury sidecar.** Create `design-jury.json` so the marketplace can show the title, description, required inputs, preview, and source repo.
...
publishing docs for the current review path. Design Jury does not store
...
| Review section IDs, copy, density, and responsive behavior | Human in the Design Jury preview |
...
If your team has a Figma export, token sync, brand kit, or deck template that keeps coming back, port the smallest repeatable slice first. Start with a `SKILL.md`, add `design-jury.json`, validate it, and open the PR before the workflow grows into a private tool nobody else can reuse. The screenshot-to-prototype example shows the plugin-shaped version: a portable skill plus an Design Jury sidecar.

[Try this workflow](https://github.com/nexu-io/design-jury/tree/main/plugins/spec/examples/import-screenshot-to-prototype).

## Related reading

- [31 skills, 72 systems: how the Design Jury library works](/blog/31-skills-72-systems-how-the-library-works/) — the primitives a plugin wraps
- [Why we built Design Jury as a skill layer, not a product](/blog/why-we-built-design-jury-as-a-skill-layer/) — why the workflow is file-shaped instead of account-shaped
