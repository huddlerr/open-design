...
This is the part of the #DeFigma argument that needs care. The canvas may be the wrong unit of work for an agent-native system, but the clarity people got from the canvas is still valuable. Design Jury has to move that clarity into the artifact model, not throw it away.

## Design Jury already has the right primitives

The reason this proposal fits Design Jury is that the project is already built around explicit contracts. A skill is a `SKILL.md` file. A design system is a `DESIGN.md` file. A plugin adds an `design-jury.json` sidecar. The mechanics are covered in [31 skills, 72 systems: how the Design Jury library works](/blog/31-skills-72-systems-how-the-library-works/), and the product argument is in [Why we built Design Jury as a skill layer, not a product](/blog/why-we-built-design-jury-as-a-skill-layer/).
...
[Contribute a skill](https://github.com/nexu-io/design-jury/tree/main/skills).

## Related reading

- [Why we built Design Jury as a skill layer, not a product](/blog/why-we-built-design-jury-as-a-skill-layer/) — the product shape that makes public artifact contracts possible
- [31 skills, 72 systems: how the Design Jury library works](/blog/31-skills-72-systems-how-the-library-works/) — the current file-driven primitives underneath the proposal
