...
>
> **正在 `main` 分支飛速迭代中** —— 0.8.0 是 Design Jury 的下一階段。提一個 PR、扔一個想法、報一個 bug —— 你帶來的，就是這場運動接下來的樣子。
>
> → [**讀公告 · 下載安裝包 · 加入這場運動**](https://github.com/nexu-io/design-jury/discussions/1727) · 可與你現有的 0.7 並行安裝。

<p align="center">
  <img src="docs/assets/banner.png" alt="Design Jury 封面：與本地 AI 智慧體共同設計" width="100%" />
</p>
...
<p align="center">
  <a href="https://design-jury.ai/"><img alt="下載客戶端" src="https://img.shields.io/badge/%E4%B8%8B%E8%BC%89-%E5%AE%A2%E6%88%B6%E7%AB%AF-ff6b35?style=flat-square" /></a>
...
**Design Jury（OD）就是它的開源替代品。** 同一套 loop、同一種「artifact-first」心智模型，但沒有鎖定。我們不做 agent —— 你筆記本上最強的 coding agent 已經裝好了。我們要做的，是把它接進一個 skill 驅動的設計工作流：本地用 `pnpm tools-dev` 跑完整本地閉環，雲端可單獨部署 Web 層，每一層都 BYOK（自帶 Key）。
...
| **部署目標** | 本地 `pnpm tools-dev` · Vercel Web 層 · 打包好的 Electron 桌面端，支援 macOS（Apple Silicon）和 Windows（x64）—— 從 [design-jury.ai](https://design-jury.ai/) 或 [最新 release](https://github.com/nexu-io/design-jury/releases) 直接下載 |
| **License** | Apache-2.0 |
...
└──────────────┬─────────────────────────────────┬───────────────┘
   ...
   │
   │  可選 sidecar IPC：/tmp/design-jury/ipc/<ns>/<app>.sock
...
試用 Design Jury 最快的方式是直接下載預編譯的桌面端 —— 不用裝 Node、不用 pnpm、不用 clone：

- **[design-jury.ai](https://design-jury.ai/)** —— 官方下載頁
- **[GitHub releases](https://github.com/nexu-io/design-jury/releases)**

### 用 Docker 執行

不需在本機安裝 Node.js 或 pnpm 就能跑 Design Jury。
...
- 已安裝的 Desktop app 寫入 `<appData>/Design Jury/namespaces/<channel>/data/`，其中 `<appData>` 是 Electron 的 per-OS app-data 基礎路徑（`app.getPath("userData")` 回傳值中 `Design Jury` 之前的部分）。channel 後綴是**平臺特定的** —— release workflow 會附加 `-win`/`-linux`：
...
適用於 Desktop app 的 `data/` 還是空的場景，這通常是升級後剛遇到 [#710](https://github.com/nexu-io/design-jury/issues/710) 的典型狀態。先結束 Desktop app（讓它的 daemon 不再佔用 `app.sqlite`），再用 `OD_LEGACY_DATA_DIR` 指向舊 repo `.od/` 來重新啟動。Daemon 會把你的 payload staging 到隔壁 tmp 目錄，成功後才 promote 進 `data/`；任何失敗都會移除 staging 目錄，下次啟動重新嘗試。
...
Design Jury 可以跑成瀏覽器裡的 web app，也可以跑成 Electron 桌面版。兩種模式共用同一套本機 daemon + web 架構。
...
```bash
nix run github:nexu-io/design-jury       # 不安裝就直接啟動 daemon（`od`）
```

開發者也有 Nix dev shell 可用，且可搭配 `direnv`：

```bash
nix develop   # 帶有開發 Design Jury 所需相依套件的 dev shell
```

## 從 coding agent 端使用 Design Jury

Design Jury 內建一個 stdio MCP server。把它接進 Claude Code、Codex、Cursor、VS Code、Antigravity、Zed、Windsurf，或任何相容 MCP 的 client，另一個 repo 裡的 agent 就能直接讀取你本機 Design Jury 專案裡的檔案。整個 export-then-attach 迴圈被取代掉。當 agent 呼叫 `search_files`、`get_file`、`get_artifact` 沒帶 project 參數時，MCP 預設指向你 Design Jury 當下開著的那個專案（與檔案）—— 所以 *「在我的 app 裡蓋這個」*、*「對齊這套樣式」* 這類提示直接就能用。
...
在 Design Jury app 裡打開 **Settings → MCP server** 就有逐 client 的安裝流程。面板會把 `node` 二進位的絕對路徑、daemon 編好的 `cli.js` 路徑，烘進每段 snippet —— 所以即使是剛 clone 下來、`od` 不在 PATH 上的環境也能用。Cursor 給一鍵 deeplink；其它 client 給可貼上的 JSON snippet（Claude Code 還附帶 `claude mcp add-json` 一行指令，不必手改 `~/.claude.json`）。裝完之後重啟或 reload 你的 client，server 才會出現。

MCP 工具呼叫成功的前提是 daemon 在本機跑著。如果 agent 是在 Design Jury 起來之前就啟動，等 OD 起來後請重啟 agent，它才連得上活的 daemon。Daemon 不在線時的工具呼叫會回 `"daemon not reachable"` 的明確錯誤，不會 crash。

**安全性。** MCP server 是唯讀的 —— 它只暴露檔案讀取、檔案 metadata、搜尋，沒有任何寫盤或呼叫外部服務的能力。它在 coding agent 下面以子行程身份透過 stdio 跑；任何你註冊上的 MCP client 都會繼承本機 Design Jury 專案的讀取權限。把它當作裝 VS Code 擴充套件那樣對待 —— 只註冊你信得過的 client。Daemon 預設綁到 `127.0.0.1`；要讓區網內的機器也能連，得明確設 `OD_BIND_HOST`。
...
[**`heygen-com/hyperframes`**](https://github.com/heygen-com/hyperframes) 是 HeyGen 開源的 agent-native 影片框架 —— 你（或 agent）寫 HTML + CSS + GSAP，HyperFrames 透過 headless Chrome + FFmpeg 確定性地渲成 MP4。Design Jury 把 HyperFrames 接成一等影片模型（`hyperframes-html`），掛進 daemon dispatch；同時帶上 `skills/hyperframes/` 這個 skill，把 timeline 合約、scene transition 規則、audio-reactive 模式、字幕 / TTS、目錄元件（`npx hyperframes add <slug>`）一起教給 agent。
...
- **Sidecar 協議 + 桌面版自動化。** Daemon、web、desktop 程序都帶型別化的 5 欄位 stamp（`app · mode · namespace · ipc · source`），並把 JSON-RPC IPC 通道暴露在 `/tmp/design-jury/ipc/<namespace>/<app>.sock`。`tools-dev inspect desktop status \| eval \| screenshot` 就跑在這條通道上，所以 headless E2E 直接打到真實 Electron 殼，不用造定製夾具（[`packages/sidecar-proto/`](packages/sidecar-proto/)、[`apps/desktop/src/main/`](apps/desktop/src/main/)）。
...
| 維度 | [Claude Design][cd]（Anthropic） | [Open CoDesign][ocod] | **Design Jury** |
|---|---|---|---|
...
- [x] `apps/packaged/` 出可分發 Electron 安裝包 —— macOS（Apple Silicon）和 Windows（x64）下載已上線 [design-jury.ai](https://design-jury.ai/) 和 [GitHub releases 頁面](https://github.com/nexu-io/design-jury/releases)
...
<p align="center">
  <a href="https://github.com/nexu-io/design-jury"><img src="docs/assets/star-us.png" alt="給 Design Jury 點個 Star —— github.com/nexu-io/design-jury" width="100%" /></a>
</p>

如果這套東西幫你省了半小時，給它一個 ★。Star 不付房租，但它告訴下一個設計師、Agent 和貢獻者：這個實驗值得他們的注意力。一次點選、三秒鐘、真實訊號：[github.com/nexu-io/design-jury](https://github.com/nexu-io/design-jury)。
...
感謝每一位讓 Design Jury 變得更好的朋友 —— 無論是寫程式碼、修文檔、提 issue、加 skill 還是加 design system，每一次真實貢獻都會被記住。下面這面牆是最直觀的「Thank you」。

<a href="https://github.com/nexu-io/design-jury/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=nexu-io/design-jury&cache_bust=2026-05-23" alt="Design Jury 貢獻者" />
</a>

第一次提 PR？歡迎從 [`good-first-issue`/`help-wanted`](https://github.com/nexu-io/design-jury/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22%2C%22help+wanted%22) 標籤起步。

## 倉庫活躍度

<picture>
  <img alt="Design Jury 倉庫指標" src="docs/assets/github-metrics.svg" />
</picture>
