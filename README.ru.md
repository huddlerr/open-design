...
>
> **Быстрая итерация на `main`** — 0.8.0 — следующая фаза Design Jury. Кидай PR, бросай безумную идею, заводи баг — что приносишь ты, таким и становится это движение.
>
> → [**Прочитать анонс · скачать установщик · присоединиться к движению**](https://github.com/nexu-io/design-jury/discussions/1727) · устанавливается параллельно с твоей текущей 0.7.

<p align="center">
  <img src="docs/assets/banner.png" alt="Design Jury — редакционная обложка: дизайн вместе с агентом на вашем ноутбуке" width="100%" />
</p>
...
<p align="center">
  <a href="https://design-jury.ai/"><img alt="Скачать" src="https://img.shields.io/badge/%D1%81%D0%BA%D0%B0%D1%87%D0%B0%D1%82%D1%8C-open--design.ai-ff6b35?style=flat-square" /></a>
...
| **Deployable to** | Локально (`pnpm tools-dev`) · web-слой на Vercel · упакованное Electron desktop-приложение для macOS (Apple Silicon) и Windows (x64) — скачать на [design-jury.ai](https://design-jury.ai/) или на [странице последнего релиза](https://github.com/nexu-io/design-jury/releases) |
| **License** | Apache-2.0 |
...
Самый быстрый способ попробовать Design Jury — готовое desktop-приложение, без Node, pnpm и клонирования:

- **[design-jury.ai](https://design-jury.ai/)** — официальная страница загрузки
- **[GitHub-релизы](https://github.com/nexu-io/design-jury/releases)**
...
│   ├── sidecar-proto/             ← протокол sidecar Design Jury
...
[**`heygen-com/hyperframes`**](https://github.com/heygen-com/hyperframes) — это open-source, agent-native video framework от HeyGen: вы (или агент) пишете HTML + CSS + GSAP, а HyperFrames рендерит всё в детерминированный MP4 через headless Chrome + FFmpeg. Design Jury поставляет HyperFrames как first-class video model (`hyperframes-html`) в dispatch-пайплайне демона, плюс skill `skills/hyperframes/`, объясняющий агенту контракт таймлайна, правила переходов между сценами, audio-reactive patterns, captions/TTS и catalog blocks (`npx hyperframes add <slug>`).
...
- **Sidecar protocol + desktop automation.** Процессы daemon, web и desktop получают типизированные пятикомпонентные stamps (`app · mode · namespace · ipc · source`) и открывают JSON-RPC IPC-канал по адресу `/tmp/design-jury/ipc/<namespace>/<app>.sock`. `tools-dev inspect desktop status \| eval \| screenshot` управляет именно этим каналом, благодаря чему headless E2E работает поверх реальной Electron-shell, без особых harness’ов ([`packages/sidecar-proto/`](packages/sidecar-proto/), [`apps/desktop/src/main/`](apps/desktop/src/main/)).
...
- [x] Packaged Electron build на базе `apps/packaged/` — загрузки для macOS (Apple Silicon) и Windows (x64) на [design-jury.ai](https://design-jury.ai/) и [странице релизов GitHub](https://github.com/nexu-io/design-jury/releases)
...
<p align="center">
  <a href="https://github.com/nexu-io/design-jury"><img src="docs/assets/star-us.png" alt="Поставьте звезду Design Jury на GitHub — github.com/nexu-io/design-jury" width="100%" /></a>
</p>

Если OD сэкономил вам хотя бы тридцать минут — подарите ему ★. Звёзды не платят аренду, но показывают следующему дизайнеру, агенту и контрибьютору, что этот эксперимент заслуживает внимания. Один клик, три секунды, реальный сигнал: [github.com/nexu-io/design-jury](https://github.com/nexu-io/design-jury).
...
Спасибо всем, кто помогает двигать Design Jury вперёд — кодом, документацией, обратной связью, новыми skills, новыми design systems или просто точным issue. Вклад любого реального масштаба здесь важен, а стена ниже — самый простой способ сказать это вслух.

<a href="https://github.com/nexu-io/design-jury/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=nexu-io/design-jury&cache_bust=2026-05-23" alt="Contributors Design Jury" />
</a>

Если вы только что отправили свой первый PR — добро пожаловать. Метка [`good-first-issue`/`help-wanted`](https://github.com/nexu-io/design-jury/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22%2C%22help+wanted%22) — хорошая точка входа.
...
<a href="https://star-history.com/#nexu-io/design-jury&Date">
  <picture>
    ...
    <img alt="История звёзд Design Jury" src="https://api.star-history.com/svg?repos=nexu-io/design-jury&type=Date&cache_bust=2026-05-23" />
  </picture>
</a>
