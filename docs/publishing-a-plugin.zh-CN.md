...
Design Jury registry v1 复用 GitHub 作为后端。CLI 是 canonical workflow；
...
公开 registry ID 必须是 `vendor/plugin-name`。生成的 `design-jury.json`
...
只需要 `SKILL.md` 和 `design-jury.json`。
...
这两个命令包装 GitHub CLI。token 留在 `gh`，Design Jury 不保存 GitHub
凭据。

## 4. 发布

```bash
od plugin publish vendor/plugin-name --to design-jury --repo https://github.com/vendor/plugin-name
```
