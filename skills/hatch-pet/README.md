...
checked into the Design Jury repo (rather than pulled in as a Git submodule
or an npm package) so that:

- Any Design Jury agent can run the skill end-to-end without a network
...
- Pinned upstream reference (declared in `SKILL.md` frontmatter): see the
  ...
  directory as the authoritative source-of-truth for Design Jury and
...
2. Replace the contents of this directory with the upstream snapshot,
   ...
   inside `SKILL.md`'s `> **Design Jury integration.**` blockquote.
...
flow (including how Design Jury behaves when Codex is not installed).
