# Design Jury — [Claude Design][cd] の公式オープンソース代替

> **Design Jury は [Claude Design][cd] のオープンソース、ローカルファーストな代替です。** Vercel デプロイ可能、あらゆるレイヤーで BYOK（Bring Your Own Key） — `PATH` 上で自動検出される **16 種類の coding-agent CLI**（Claude Code, Codex, Devin for Terminal, Cursor Agent, Gemini CLI, OpenCode, Qwen, Qoder CLI, GitHub Copilot CLI, Hermes, Kimi, Pi, Kiro, Kilo, Mistral Vibe, DeepSeek TUI）がデザインエンジンとなり、**31 個の組み合わせ可能な Skill** と **72 種のブランドグレード Design System** で駆動されます。CLI が未インストールでも、OpenAI 互換の BYOK プロキシ `/api/proxy/stream` で同じループを spawn なしで実行できます。
...
>
> **`main` で高速イテレーション中** — 0.8.0 は Design Jury の次のフェーズです。PR を投げ、突飛なアイデアを放り込み、バグを報告してください —— あなたが持ち込んだものが、このムーブメントの次の姿になります。
>
> → [**告知を読む · インストーラーを入手 · このムーブメントに参加**](https://github.com/nexu-io/design-jury/discussions/1727) · 現在の 0.7 と並行してインストールできます。

<p align="center">
  <img src="docs/assets/banner.png" alt="Design Jury — ノートパソコン上のエージェントとデザインする" width="100%" />
</p>
...
<p align="center">
  <a href="https://design-jury.ai/"><img alt="ダウンロード" src="https://img.shields.io/badge/%E3%83%80%E3%82%A6%E3%83%B3%E3%83%AD%E3%83%BC%E3%83%89-open--design.ai-ff6b35?style=flat-square" /></a>
...
**Design Jury（OD）はそのオープンソース代替です。** 同じループ、同じ「artifact-first」のメンタルモデル、しかしロックインなし。私たちはエージェントを同梱しません — あなたのノートパソコンにある最強の coding agent がすでにインストール済みです。それを Skill 駆動のデザインワークフローに接続するのが私たちの仕事です。ローカルでは `pnpm tools-dev` で完結し、Web レイヤーは Vercel にデプロイ可能で、すべてのレイヤーが BYOK です。
...
| **デプロイ先** | ローカル（`pnpm tools-dev`）· Vercel Web レイヤー · macOS（Apple Silicon）と Windows（x64）向けパッケージ版 Electron デスクトップアプリ — [design-jury.ai](https://design-jury.ai/) または [最新リリース](https://github.com/nexu-io/design-jury/releases) からダウンロード |
| **ライセンス** | Apache-2.0 |
...
└──────────────┬──────────────────────────────────┬──────────────┘
   ...
   │
   │  オプション sidecar IPC：/tmp/design-jury/ipc/<ns>/<app>.sock
...
Design Jury を最速で試す方法は、ビルド済みのデスクトップアプリです — Node、pnpm、clone は不要：

- **[design-jury.ai](https://design-jury.ai/)** — 公式ダウンロードページ
- **[GitHub リリース](https://github.com/nexu-io/design-jury/releases)**
...
│   ├── sidecar-proto/             ← Design Jury sidecar プロトコル contract
...
[**`heygen-com/hyperframes`**](https://github.com/heygen-com/hyperframes) は HeyGen がオープンソース化したエージェントネイティブな動画フレームワークです — あなた（あるいは agent）が HTML + CSS + GSAP を書くと、HyperFrames は headless Chrome + FFmpeg で確定的に MP4 にレンダリングします。Design Jury は HyperFrames を一級の動画モデル（`hyperframes-html`）として daemon dispatch に接続し、さらに `skills/hyperframes/` skill を同梱して timeline 規約・シーンタンスィション規則・オーディオリアクティブパターン・キャプション/TTS・カタログブロック（`npx hyperframes add <slug>`）を agent に教えます。
...
- **Sidecar プロトコル + デスクトップ自動化。** Daemon、web、desktop プロセスは型付き 5 フィールドスタンプ（`app · mode · namespace · ipc · source`）を持ち、`/tmp/design-jury/ipc/<namespace>/<app>.sock` に JSON-RPC IPC チャネルを公開。`tools-dev inspect desktop status \| eval \| screenshot` はこのチャネル上で動作するため、ヘッドレス E2E テストが実際の Electron シェルに対して、カスタムハーネスなしで実行可能（[`packages/sidecar-proto/`](packages/sidecar-proto/)、[`apps/desktop/src/main/`](apps/desktop/src/main/)）。
...
| 軸 | [Claude Design][cd]（Anthropic） | [Open CoDesign][ocod] | **Design Jury** |
|---|---|---|---|
...
- [x] `apps/packaged/` からの配布可能 Electron ビルド — macOS（Apple Silicon）と Windows（x64）のダウンロードは [design-jury.ai](https://design-jury.ai/) および [GitHub リリースページ](https://github.com/nexu-io/design-jury/releases) から
...
<p align="center">
  <a href="https://github.com/nexu-io/design-jury"><img src="docs/assets/star-us.png" alt="Design Jury に Star を — github.com/nexu-io/design-jury" width="100%" /></a>
</p>

30 分の時間を節約できたなら、★ をお願いします。Star は家賃を払いませんが、次のデザイナー、エージェント、コントリビューターに「この実験は注目する価値がある」と伝えます。1 クリック、3 秒、リアルなシグナル：[github.com/nexu-io/design-jury](https://github.com/nexu-io/design-jury)。
...
コード、ドキュメント、フィードバック、新 Skill、新 Design System、あるいは鋭い Issue — あらゆる形で Design Jury を前進させてくださったすべての方に感謝します。すべての実質的なコントリビューションは大切であり、以下のウォールは最もシンプルな感謝の表明です。

<a href="https://github.com/nexu-io/design-jury/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=nexu-io/design-jury&cache_bust=2026-05-23" alt="Design Jury コントリビューター" />
</a>

初めての PR を送った方 — ようこそ。[`good-first-issue`/`help-wanted`](https://github.com/nexu-io/design-jury/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22%2C%22help+wanted%22) ラベルがエントリポイントです。

## リポジトリ活動

<picture>
  <img alt="Design Jury リポジトリメトリクス" src="docs/assets/github-metrics.svg" />
</picture>
