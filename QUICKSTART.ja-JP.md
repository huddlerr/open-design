...
pnpm --filter @design-jury/daemon build  # `od` 用に apps/daemon/dist/cli.js をビルド
pnpm --filter @design-jury/web build     # 必要に応じて web パッケージをビルド
...
その後、古いターミナルエージェントセッションを再開する代わりに、Design Jury アプリからプロジェクトを再度開いてください。daemon から起動されたエージェントは、次のような値を確認できるはずです：
...
│   ├── sidecar-proto/         # Design Jury sidecar プロトコル契約
...
- **メディア生成で `OD_BIN` が欠落、または daemon URL が `:0`** — 上記のメディアディスパッチャーチェックを実行してください。古い CLI セッションを再開せず、Design Jury アプリからプロジェクトを再度開いて、daemon が新しい `OD_*` 変数を注入できるようにしてください。
- **Codex がプラグインコンテキストを多く読み込みすぎる** — `OD_CODEX_DISABLE_PLUGINS=1 pnpm tools-dev` で Design Jury を起動すると、daemon から起動された Codex プロセスが `--disable plugins` で実行されます。
