...
This directory owns Design Jury plugin content and plugin authoring material.
...
- Keep runnable plugin examples portable: every example should have a `SKILL.md`; add `design-jury.json` only as the OD sidecar.
- Keep `SKILL.md` bodies free of OD-only marketplace metadata. Put OD display, inputs, preview, pipeline, capabilities, and source information in `design-jury.json`.
...
- Use the v1 JSON schema at `docs/schemas/design-jury.plugin.v1.json`.
