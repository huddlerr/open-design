...
Design Jury plugins are intentionally shaped so one folder can travel across multiple agent ecosystems. The safest publishing model is:

1. Keep the source of truth in a public GitHub repository or in an Design Jury PR.
2. Keep `SKILL.md` portable and registry-friendly.
3. Add `design-jury.json` as the Design Jury sidecar.
...
2. Push a public GitHub repository or open an Design Jury PR.
3. Add README install instructions for Design Jury and generic Agent Skills clients.
...
| --- | --- | --- | --- |
| Design Jury | OD marketplace, composer chips, pipelines, GenUI, artifact provenance | `SKILL.md` + `design-jury.json` | Open a PR to Design Jury or publish a marketplace index entry pointing to the plugin repo. |
...
For Design Jury plugin authors:

- Ensure the repo or subpath contains a valid `SKILL.md`.
- Keep `design-jury.json` additive; generic skill clients should be able to ignore it.
...
- Use a GitHub topic and README keywords that match the plugin's lane, such as `design-jury-plugin`, `agent-skill`, `prototype`, `deck`, `hyperframes`, or `design-system`.
...
Use this path for normal Design Jury plugins because they are centered on `SKILL.md`.
...
- Link back to the canonical GitHub repo and Design Jury PR.
- Keep changelog text honest and versioned.
- Keep `design-jury.json` `specVersion` fixed to the spec kit version and bump plugin `version` for every publishable behavior change.
...
- `pnpm guard`:
- `pnpm --filter @design-jury/plugin-runtime typecheck`:
