...
Design Jury 插件刻意设计成一个文件夹可以跨多个 agent 生态流转。最稳妥的发布模型是：

1. 把公开 GitHub 仓库或 Design Jury PR 作为 source of truth。
2. 保持 `SKILL.md` 可移植、适合 registry 读取。
3. 添加 `design-jury.json` 作为 Design Jury sidecar。
...
1. 本地验证插件文件夹。
2. 推送公开 GitHub 仓库，或向 Design Jury 打开 PR。
3. 在 README 中添加 Design Jury 和通用 Agent Skills 客户端的安装说明。
...
| 目标 | 适合场景 | 源形态 | 发布策略 |
| --- | --- | --- | --- |
| Design Jury | OD marketplace、composer chips、pipelines、GenUI、artifact provenance | `SKILL.md` + `design-jury.json` | 向 Design Jury 提 PR，或发布指向插件仓库的 marketplace index entry。 |
...
- 保持 `design-jury.json` 为纯增量；通用 skill 客户端应能忽略它。
- 在 README 中放一个短安装块：
...
- 使用与插件主类匹配的 GitHub topic 和 README 关键词，例如 `design-jury-plugin`、`agent-skill`、`prototype`、`deck`、`hyperframes`、`design-system`。
...
普通 Design Jury 插件应优先走这个路径，因为它们以 `SKILL.md` 为核心。
...
- 链回 canonical GitHub repo 和 Design Jury PR。
- changelog 诚实且版本化。
- `design-jury.json` 的 `specVersion` 保持为规范包版本；每次可发布的行为变化都 bump 插件 `version`。
