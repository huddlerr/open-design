...
Give this file to a coding agent when you want it to create or improve an Design Jury plugin.

## Mission

Create a portable Design Jury plugin that can:

1. Run as a normal Agent Skill through `SKILL.md`.
2. Install into Design Jury through `design-jury.json`.
3. Be validated locally.
4. Be published as an independent open source repo or submitted as a PR to Design Jury.
...
- `docs/schemas/design-jury.plugin.v1.json`
- `docs/plugins-spec.md` when you need deeper product semantics
- `plugins/spec/PUBLISHING-REGISTRIES.md` when the user asks to publish outside Design Jury
...
3. Keep the `SKILL.md` portable. It may mention Design Jury behavior, but the core workflow must still make sense in any Agent Skills compatible agent.
4. Put OD-specific display, `specVersion`, plugin `version`, inputs, preview, pipeline, atoms, connectors, and capabilities in `design-jury.json`.
...
- `design-jury.json` validates against the v1 shape and carries explicit `specVersion` plus plugin `version`.
