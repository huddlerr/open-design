...
Create an Design Jury live artifact: a project-scoped, previewable HTML artifact whose data can later be refreshed without redesigning the presentation.
...
5. **Register or update through daemon wrappers**
   - Use the Design Jury daemon wrapper commands via `"$OD_NODE_BIN" "$OD_BIN"` instead of raw `curl`, bare `node`, or bare `od`:
