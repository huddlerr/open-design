...
When this skill runs inside Design Jury, the daemon shells out to `npx hyperframes render`, which inherits the auto-enable. You do **not** need to add browser flags or pass extra CLI args from the agent.
...
For Docker renders, the flag is also enabled automatically inside the container. Inside Design Jury, the daemon's `npx hyperframes render` call (`apps/daemon/src/media.ts`) inherits the same default — you don't need to thread anything through.
