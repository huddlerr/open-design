...
pnpm --filter @design-jury/daemon build  # build apps/daemon/dist/cli.js pour `od`
...
Ouvrez ensuite de nouveau le projet depuis l’app Design Jury au lieu de reprendre une ancienne session agent dans le terminal. Un agent lancé par le daemon devrait voir des valeurs comme :
...
│   ├── sidecar-proto/         # contrat du protocole sidecar Design Jury
...
- **la génération média dit que `OD_BIN` manque ou que l’URL daemon vaut `:0`** — exécutez les checks du dispatcher média ci-dessus. Ne reprenez pas l’ancienne session CLI ; rouvrez le projet depuis l’app Design Jury pour que le daemon injecte des variables `OD_*` fraîches.
- **Codex charge trop de contexte plugin** — démarrez Design Jury avec `OD_CODEX_DISABLE_PLUGINS=1 pnpm tools-dev` pour que les processus Codex lancés par le daemon tournent avec `--disable plugins`.
